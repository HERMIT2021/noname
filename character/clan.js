...
const skills = {
  ...
  clanqingjue: {
    ...
    async content(event2, trigger2, player2) {
      const result2 = await player2.chooseToDiscard(`###${get.translation(event2.name)}###弃置手牌中任意张花色数量不为一的牌，并执行等量项`, "h", [1, Infinity], true, "allowChooseAll").set("filterCard", (card, player3) => !get.info("clanqingjue").isOnlySuit(card, player3)).forResult();
      ...
      const resultx = cards2.length > 1 ? { links: ["give", "gain"] } : await player2.chooseButton(
        ...
        if (button.link == "give") {
          if (game.hasPlayer((target2) => target2 != get.player() && get.attitude(get.player(), target2) > 0)) {
            return 2;
          }
          return 0.5;
        }
      ...
      const { links } = resultx;
      if (links?.includes("give") && game.hasPlayer((target2) => target2 != player2) && cards2?.someInD("d")) {
        const toGive = cards2?.filterInD("d");
        const result3 = await player2.chooseTarget(true, lib.filter.notMe).set("createDialog", [`清绝：将这些牌交给一名其他角色`, toGive, [(dialog) => dialog.buttons.forEach((button) => button.style.setProperty("opacity", "1", "important")), "handle"]]).set("toGive", toGive).set("ai", (target3) => get.attitude(get.player(), target3) * get.value(get.event().toGive, target3)).forResult();
        const {
          targets: [target2]
        } = result3;
        player2.line(target2);
        const next = target2.gain(toGive, "gain2");
        next.giver = player2;
        await next;
      }
      ...
      mark: {
        ...
        getIndex(event2, player2) {
          return game.filterPlayer((target2) => {
            ...
            const gain = event2.getg?.(target2) ?? [];
        ...
        async content(event2, trigger2, player2) {
          ...
          const gain = trigger2.getg?.(target2);
          const lose = trigger2.getl?.(player2)?.cards2;
          target2.addGaintag(
            gain.filter((i) => trigger2.giver === player2 || lose.includes(i)),
            "clanxsyingxiang"
          );
        }
      }
  ...
  clanfenjian: {
    ...
    onChooseToUse(event2) {
      ...
      if (!game.online && !event2.clanfenjian) {
        ...
        event2.targetprompt2.push((target2) => {
          if (!target2.classList.contains("selectable") || event2.skill !== "clanfenjian") {
            return false;
          }
          const num = target2.getAttackRange() - get.event().player.getAttackRange();
          if (num < 0) {
            return "小于你";
          } else if (num > 0) {
            return "大于你";
    ...
    filter(event2, player2) {
      ...
      return game.hasPlayer((target2) => {
        return !player2.getStorage(name).includes(Math.sign(target2.getAttackRange() - player2.getAttackRange())) && target2.countDiscardableCards(target2, "he") > 0;
      });
    },
    filterTarget(card, player2, target2) {
      return !player2.getStorage("clanfenjian_used").includes(Math.sign(target2.getAttackRange() - player2.getAttackRange())) && target2.countDiscardableCards(target2, "he") > 0;
    },
    async content(event2, trigger2, player2) {
      const { target: target2 } = event2;
      const num = Math.sign(target2.getAttackRange() - player2.getAttackRange());
      player2.addTempSkill(`${event2.name}_used`, "phaseChange");
      player2.markAuto(`${event2.name}_used`, num);
      const result2 = await target2.chooseToDiscard(`奋剑：你须弃置一张牌，若为【闪】，${get.translation(player2)}摸一张牌，否则其本回合攻击范围-1`, "he", true).forResult();
      const {
        cards: [card]
      } = result2;
    ...
    subSkill: {
      yang: {
        ...
        filterTarget(card, player2, target2) {
          ...
          return target2.canEquip(cardx, true) && lib.filter.filterTarget(get.autoViewAs(get.info("clandongxu_yang").viewAs), player2, target2);
        },
        async precontent(event2, trigger2, player2) {
          ...
          const [target2] = event2.result.targets;
          event2.result.cards = [];
          event2.result.card = { name: "sha", isCard: true };
          player2.$give(card, target2, false);
  ...
  clangezhi: {
    ...
    logTarget: "source",
    async assignCards(event2, trigger2, player2) {
      ...
      if (event2.type == "card") {
        ...
        while (assigned.length < cards2.length && goon(event2)) {
          ...
          if (result2?.bool && result2.links?.length && result2.targets?.length) {
            ...
            give_map.set(target2, (give_map.get(target2) || []).concat(links));
          } else {
            break;
          }
        }
      ...
      if (!chooseonly) {
        ...
        if (event2.type == "button") {
          for (const target2 of Array.from(give_map.keys()).sortBySeat()) {
            game.log(target2, "获得了", give_map.get(target2));
          }
        }
    ...
    async content(event2, trigger2, player2) {
      ...
      if (player2.countDiscardableCards(player2, "he")) {
        const result2 = await player2.chooseToDiscard("he", true).forResult();
        if (result2?.cards?.length) {
          cards2.addArray(result2.cards);
        }
      }
      if (target2.countDiscardableCards(player2, "he")) {
        const result2 = await player2.discardPlayerCard(target2, "he", true).forResult();
        if (result2?.cards?.length) {
          cards2.addArray(result2.cards);
        }
      }
  ...
  clanshize: {
    ...
    filter(event2, player2) {
      ...
      return history.indexOf(event2) == 0 && game.hasPlayer((target2) => target2.hasClan("颍川陈氏") || target2 == player2);
    },
    async cost(event2, trigger2, player2) {
      event2.result = await player2.chooseTarget(`###${get.translation(event2.skill)}###${get.skillInfoTranslation(event2.skill, player2)}`, true, (card, player3, target2) => {
        return target2.hasClan("颍川陈氏") || target2 == player3;
      }).set("ai", (target2) => {
        const player3 = get.player();
        const att = get.attitude(player3, target2);
        if (att > 0) {
          return game.countPlayer((i) => !i.inRangeOf(target2)) + 1;
        }
        return 0;
      }).forResult();
    },
    async content(event2, trigger2, player2) {
      ...
      game.log(target2, "的攻击范围", "#g+1");
      target2.addSkill(`${event2.name}_effect`);
      target2.addMark(`${event2.name}_effect`, 1, false);
    },
    subSkill: {
      effect: {
        charlotte: true,
  ...
  clandingan: {
    ...
    filter(event2, player2) {
      return game.getGlobalHistory("useCard", (evt) => evt.card.name == event2.card.name).indexOf(event2) == 1;
    },
    forced: true,
    async content(event2, trigger2, player2) {
      const skill = `${event2.name}_used`;
      const pre_targets = game.filterPlayer((target2) => {
        return !trigger2.targets.includes(target2) && !player2.getStorage(skill).includes(target2);
      });
      const result2 = pre_targets.length > 1 ? await player2.chooseTarget(
      ...
      ).set("targetx", pre_targets).set("prompt2", "然后你令手牌最多的其他角色执行一项：1.受到你造成的1点伤害；2.弃置手牌中最多的同名牌。").set("ai", (target2) => {
      ...
      player2.markAuto(skill, result2.targets);
      const targets2 = [player2, ...result2.targets.sortBySeat()];
      ...
      const func = async (target2) => {
        const result22 = await player2.chooseButton(
          [
            `定安：选择一项令${get.translation(target2)}执行`,
            [
              [
                ["damage", "受到你造成的1点伤害"],
          ...
          true
        ).set("targetx", target2).set("ai", (button) => {
          const { player: player3, targetx: target3 } = get.event();
          if (button.link == "damage") {
            return get.damageEffect(target3, player3, player3);
          }
          return get.sgnAttitude(player3, target3) * Math.sqrt(target3.countCards("h"));
        }).forResult();
        if (result22?.bool && result22.links?.length) {
          player2.line(target2);
          if (result22.links[0] == "damage") {
            await target2.damage(player2);
          } else {
            const cards2 = target2.getCards("h"), names = cards2.map((card) => get.name(card)), maxName = names.toUniqued().maxBy((name2) => get.numOf(names, name2));
            ...
            if (name) {
              ...
              if (discards?.length) {
                await target2.modedDiscard(discards);
              }
            }
          }
        }
    ...
    subSkill: {
      effect: {
        ...
        filter(event2, player2) {
          ...
          const targets2 = player2.getHistory(
            "useCard",
            (evt) => {
          ...
          return targets2.some((target2) => {
            if (event2.targets?.includes(target2)) {
              return true;
            }
        ...
        async cost(event2, trigger2, player2) {
          const targets2 = player2.getHistory(
            "useCard",
            (evt) => {
              return evt?.targets?.length && evt != trigger2;
            },
            trigger2
          ).map((evt) => evt.targets ?? []).flat().toUniqued();
          event2.result = await player2.chooseTarget(
            (card, player3, target2) => {
              ...
              if (!targetx.includes(target2)) {
                return false;
              }
              if (ui.selected.targets?.length) {
                const first = ui.selected.targets[0];
                if (targety.includes(first) !== targety.includes(target2)) {
                  return false;
                }
              }
              return targety.includes(target2) || lib.filter.targetEnabled2(cardx, player3, target2);
            },
            [1, Infinity]
          ).set("targetx", targets2).set("targety", trigger2.targets).set("cardx", trigger2.card).set("complexTarget", true).set("targetprompt", (target2) => {
            const { targety } = get.event();
            return targety.includes(target2) ? "取消目标" : "成为目标";
          }).set("prompt", get.translation(event2.skill)).set("prompt2", "为此牌增加或减少任意个目标").set("ai", (target2) => {
            const { targety, cardx, player: player3 } = get.event(), eff = get.effect(target2, cardx, player3, player3);
            if (targety.includes(target2)) {
              return -eff * get.attitude(player3, target2);
            }
            return eff * get.attitude(player3, target2);
          }).forResult();
        },
        async content(event2, trigger2, player2) {
          ...
          const bool = trigger2.targets.containsSome(...event2.targets);
          trigger2.targets[bool ? "removeArray" : "addArray"](event2.targets);
          await game.delay();
        }
      }
    }
  ...
  clanyingxiang: {
    ...
    async content(event2, trigger2, player2) {
      ...
      if (!result2.tie) {
        const winner = result2.bool ? player2 : target2, card = new lib.element.VCard({ name, isCard: true });
        if (winner.hasUseTarget(card)) {
          await winner.chooseUseTarget(card, true);
        }
      }
      const cards2 = [result2.player, result2.target];
      if (cards2.some((card) => card.name == name)) {
        ...
        const result22 = skills2.length > 1 ? await player2.chooseButton(["迎乡：失去一个技能", [skills2, "skill"]], true).set("ai", (button) => {
          const { link } = button;
        ...
        if (result22?.bool && result22?.links?.length) {
          await player2.removeSkills(result22.links);
        }
      }
    }
  },
  ...
  clanfennu: {
    ...
    async cost(event2, trigger2, player2) {
      event2.result = await player2.chooseTarget(get.prompt2(event2.skill), [1, player2.hp]).set("filterTarget", (card, player3, target2) => {
        return target2.countCards("he");
      }).set("ai", (target2) => {
        const player3 = get.player();
        if (target2.hasSkill("clanzelie") && target2.countCards("e") == 1 && !target2.countCards("j")) {
          return get.attitude(player3, target2);
        }
    ...
    async content(event2, trigger2, player2) {
      const targets2 = event2.targets.filter((target2) => target2.countDiscardableCards(target2, "he"));
      if (!targets2?.length) {
        return;
      }
      ...
      const nextx = player2.chooseCardOL(targets2, "he", true, 1, "奋驽：弃置一张牌", (card, player3, target2) => {
        return lib.filter.cardDiscardable(card, player3, "clanfennu");
      }).set("ai", (card) => {
        return 7 - get.value(card);
      });
      nextx._args.remove("glow_result");
      const result2 = await nextx.forResult();
      const cards2 = [];
      for (let i = 0; i < result2.length; i++) {
        const current = targets2[i], cards22 = result2[i].cards;
    ...
    subSkill: {
      record: {
        ...
        direct: true,
        async content(event2, trigger2, player2) {
          player2.addMark(event2.name, get.number(trigger2.card), false);
  ...
  clanzelie: {
    ...
    async content(event2, trigger2, player2) {
      ...
      target2.addTempSkill("clanzelie_effect");
      target2.addMark("clanzelie_effect", 1, false);
    },
    ai: {
      noe: true,
      skillTagFilter(player2, tag, arg) {
  ...
  clantanfeng: {
    ...
    async precontent(event2, trigger2, player2) {
      ...
      player2.when({ player: "useCardAfter" }).filter((evt) => evt.getParent() == event2.getParent()).assign({
        ...
        if (player3.getHistory("sourceDamage", (evt) => evt.card == trigger3.card).length) {
          const num = Math.abs(player3.countCards("e") - target2.countCards("e"));
          if (num > 0) {
            await player3.draw(num);
          }
        } else {
          const sha = get.autoViewAs({ name: "sha", isCard: true });
          if (!target2.canUse(sha, player3, false)) {
            return;
          }
          const { bool } = await target2.chooseBool("探锋", `是否视为对${get.translation(player3)}使用一张【杀】？`).set("choice", get.effect(player3, sha, target2, target2) > 0).forResult();
          if (bool) {
            await target2.useCard(sha, player3, false);
          }
        }
      });
    },
  ...
  clanjuewei: {
    ...
    usable: 1,
    async cost(event2, trigger2, player2) {
      const prompt1 = `重铸一张装备牌，于${get.translation(trigger2.card)}结算完成后视为对一名不为你的角色使用此牌`, prompt2 = `弃置一张装备牌，令${get.translation(trigger2.card)}无效`;
      const result2 = await player2.chooseButton([
        ...
        if (button.link == "Recast") {
          if (!trigger3.targets || trigger3.targets.every((target3) => target3 == player3)) {
            return 0;
          }
          const target2 = trigger3.targets.maxBy(
            (target3) => {
              return get.effect(target3, trigger3.card, player3, player3);
            },
            (target3) => target3 != player3
          );
          return get.effect(target2, trigger3.card, player3, player3);
        }
        return trigger3.targets.reduce((sum, target2) => {
          sum -= get.effect(target2, trigger3.card, trigger3.player, player3);
        }, 0) - 2;
      }).forResult();
      if (!result2.bool) {
        return;
      }
      const link = result2.links[0], type = `card${link}able`;
    ...
    async content(event2, trigger2, player2) {
      if (event2.cost_data == "Recast") {
        ...
        player2.when({ global: "useCardAfter" }).filter((evt) => evt == trigger2.getParent()).step(async (event3, trigger3, player3) => {
          const targets2 = trigger3.targets.filter((target2) => target2 != player3 && target2.isIn());
          if (targets2?.length) {
            await player3.chooseUseTarget(trigger3.card, false, true, targets2, 1);
          }
        });
      } else {
        await player2.discard(event2.cards);
        trigger2.getParent().all_excluded = true;
        trigger2.getParent().targets.length = 0;
      }
    }
  },
  //族杨彪
  clanjiannan: {
    ...
    subSkill: {
      ...
      effect: {
        ...
        onremove(player2) {
          game.filterPlayer().forEach((target2) => target2.removeGaintag("clanjiannan"));
        },
        charlotte: true,
        ...
        getIndex(event2, player2, triggername) {
          const targets2 = game.filterPlayer((target2) => {
            const evt = event2.getl(target2);
            if (!target2.countCards("h") && evt?.hs?.length) {
              return true;
            }
            if (!evt?.cards2?.length || target2.countCards("h", (card) => card.hasGaintag("clanjiannan"))) {
              return false;
            }
            if (event2.name == "lose") {
              return Object.values(event2.gaintag_map).flat().includes("clanjiannan");
            }
            return target2.hasHistory("lose", (evtx) => {
        ...
        async cost(event2, trigger2, player2) {
          event2.result = await player2.chooseButtonTarget({
              ...
              switch (button.link) {
                case "discard": {
                  if (game.hasPlayer((target2) => {
                    const att = get.attitude(player3, target2);
                    return att < 0 && target2.countCards("he");
                  })) {
                    return 2;
                  }
                  break;
                ...
                case "put": {
                  if (game.hasPlayer((target2) => {
                    const att = get.attitude(player3, target2);
                    return att < 0 && target2.hp <= 1 && target2.countCards("h") <= 3;
                  })) {
                    return 5;
                  }
                  break;
            ...
            ai2: (target2) => {
              const link = ui.selected.buttons[0]?.link, player3 = get.player(), att = get.attitude(player3, target2);
              if (!link) {
                return 0;
              }
        ...
        async content(event2, trigger2, player2) {
          ...
          switch (link) {
            ...
            case "draw": {
              const next = target2.draw(2);
              next.gaintag = ["clanjiannan"];
              await next;
              break;
            }
            case "recast": {
              await target2.recast(
                target2.getCards("he", (card) => target2.canRecast(card) && get.type(card, player2) == "equip"),
                null,
                (player3, cards2) => {
            ...
            case "put": {
              ...
              if (result2.bool) {
                target2.$throw(result2.cards.length);
                game.log(target2, "将", result2.cards, "置于牌堆顶");
                await target2.lose(result2.cards, ui.cardPile, "insert");
              } else {
  ...
  clanyichi: {
    ...
    async cost(event2, trigger2, player2) {
      event2.result = await player2.chooseTarget(get.prompt2(event2.skill), (card, player3, target2) => player3.canCompare(target2)).set("ai", (target2) => {
        const player3 = get.player(), num = player3.getHistory("useSkill", (evt) => ["clanjiannan", "clanjiannan_effect"].includes(evt.skill)).length;
        return num > 1 ? get.attitude(player3, target2) : -get.attitude(player3, target2);
      }).forResult();
    },
    async content(event2, trigger2, player2) {
      const target2 = event2.targets[0], { bool } = await player2.chooseToCompare(target2).forResult(), num = player2.getHistory("useSkill", (evt) => ["clanjiannan", "clanjiannan_effect"].includes(evt.skill)).length;
      if (bool && num > 0) {
        let count = 1;
        while (count < 5) {
          switch (count) {
            ...
            case 3: {
              await target2.recast(target2.getCards("he", (card) => target2.canRecast(card) && get.type(card, player2) == "equip"));
              break;
            }
            case 4: {
              const result2 = await target2.chooseCard(
  ...
  clanjuetu: {
    ...
    forced: true,
    async content(event2, trigger2, player2) {
      trigger2.setContent(lib.skill[event2.name].phaseDiscard);
    },
    phaseDiscard: [
      ...
      async (event2, trigger2, player2) => {
        ...
        if (lib.suits.some((suit) => suits.filter((suitx) => suitx == suit).length > 1)) {
          const result2 = await player2.chooseCard("请保留每种花色的手牌各一张，将其余手牌置入弃牌堆", [1, Infinity], true, (card, player3) => {
          ...
          const discard = cards2.removeArray(result2.cards);
          await player2.loseToDiscardpile(discard);
        }
        if (game.hasPlayer((target2) => target2.countDiscardableCards(target2, "h"))) {
          const result2 = await player2.chooseTarget(`绝途：令一名角色弃置一张手牌`, true, (card, player3, target3) => {
            return target3.countDiscardableCards(target3, "h");
          }).set("ai", (target3) => {
            return get.effect(target3, { name: "guohe_copy", position: "h" }, target3, get.player());
          }).forResult();
          ...
          next.setContent(async (event3, trigger3, player3) => {
            const result3 = await target2.chooseToDiscard("绝途：请弃置一张手牌", true, "h").forResult();
            if (!result3?.cards?.length) {
              event3.finish();
              return;
            }
            const suit = get.suit(result3.cards[0], target2);
  ...
  clankudu: {
    ...
    async content(event2, trigger2, player2) {
      ...
      const cards2 = event2.cards, num = Math.min(5, Math.abs(get.number(cards2[0]) - get.number(cards2[1]))), target2 = event2.targets[0], skill = event2.name + "_effect";
      await player2.recast(cards2);
      if (num > 0) {
        target2.addSkill(skill);
        target2.addMark(skill, num, false);
      }
    },
    subSkill: {
      effect: {
  ...
  clanyangji: {
    ...
    async content(event2, trigger2, player2) {
      ...
      if (!player2.hasHistory("sourceDamage", (evt) => evt.getParent(next.name) == next) && target2?.canAddJudge("lebu")) {
        await player2.chooseToUse().set("openskilldialog", `佯疾：是否将一张黑桃牌当作【乐不思蜀】对${get.translation(target2)}使用？`).set("norestore", true).set("_backupevent", `${event2.name}_backup`).set("custom", {
          add: {},
          replace: { window() {
          } }
        }).backup(`${event2.name}_backup`).set("targetRequired", true).set("complexTarget", true).set("complexSelect", true).set("addCount", false).set("targetx", target2);
      }
    },
    subSkill: {
      backup: {
        ...
        filterTarget(card, player2, target2) {
          return lib.filter.judge(card, player2, target2) && target2 === get.event().targetx;
        },
        filterOk() {
        ...
        ai2(target2) {
          if (target2 == get.player() && get.player().hasSkill("clandandao")) {
            return true;
          }
  ...
  clandandao: {
    ...
    content() {
      ...
      target2.addSkill(event.name + "_add");
      target2.addMark(event.name + "_add", 1, false);
    },
    subSkill: {
      add: {
        charlotte: true,
  ...
  clanjiewu: {
    ...
    async cost(event2, trigger2, player2) {
      event2.result = await player2.chooseTarget(get.prompt(event2.skill), "令一名角色的手牌在本阶段对你可见").set("ai", (target2) => {
        let items = target2.getCards("h");
        let count = [...new Set(items.map((item) => get.suit(item, target2)))].length;
        const player3 = get.player();
        return get.effect(target2, { name: "draw" }, target2, player3) * items / (count + 1);
      }).forResult();
    },
    async content(event2, trigger2, player2) {
      const target2 = event2.targets[0];
      ...
      target2.addSkill(event2.name + "_view");
      const func = (target3) => target3.markSkill("clanjiewu_view", null, null, true);
      event2.isMine() ? func(target2) : player2.isOnline2() && player2.send(func, target2);
      player2.when({ global: "phaseUseAfter" }).filter((evt) => evt === trigger2).step(async () => player2.removeSkill("clanjiewu_effect"));
    },
    subSkill: {
      effect: {
        charlotte: true,
        ...
        filter: (event2, player2) => event2.isFirstTarget && event2.targets.some((target2) => target2 != player2),
        async cost(event2, trigger2, player2) {
          event2.result = await player2.chooseTarget(get.prompt(event2.skill), "选择一名「捷悟」角色展示其一张手牌").set("filterTarget", (card, player3, target2) => target2.hasCard(true, "h") && player3.getStorage("clanjiewu_effect").includes(target2)).set("ai", (target2) => {
            let items = target2.getCards("h");
            let count = [...new Set(items.map((item) => get.suit(item, target2)))].length;
            const player3 = get.player();
            return (4 - count) * get.effect(target2, { name: "draw" }, target2, player3);
          }).forResult();
        },
        async content(event2, trigger2, player2) {
          const target2 = event2.targets[0];
          ...
          if (target2 === player2) {
            ...
            cards2 = (await player2.choosePlayerCard(target2, true, "h", `捷悟：展示${get.translation(target2)}的一张手牌`).forResult()).cards;
          }
          if (!cards2?.length) {
            return;
          }
          const card = cards2[0];
          await player2.showCards(card, `${get.translation(player2)}对${get.translation(target2)}发动了【捷悟】`).set("clanjiewu", true);
          if (get.suit(trigger2.card, player2) === get.suit(card, target2)) {
            await player2.draw();
          }
          if (game.getGlobalHistory("everything", (evt) => {
            ...
            if (target2.countCards("h") !== player2.countCards("h")) {
              const putee = player2.countCards("h") > target2.countCards("h") ? player2 : target2;
  ...
  clanqieyi: {
    ...
    async content(event2, trigger2, player2) {
      if (trigger2.name == "phaseUse") {
        ...
        if (get.color(card) == get.color(trigger2.card) || get.type2(card) == get.type2(trigger2.card)) {
          await player2.gain(card, "gain2");
        } else {
          if (!player2.countCards("he")) {
            return;
          }
          const result2 = await player2.chooseCard(`切议：将一张牌置于牌堆顶`, "he", true).forResult();
  ...
  clanjianzhi: {
    ...
    async content(event2, trigger2, player2) {
      ...
      const result2 = await player2.chooseControl(list).set("prompt", `###谏直###进行至多${get.cnNumber(list.length)}次判定，并执行后续效果`).set("ai", () => get.event().list.length - 1).set("list", list).forResult();
      if (typeof result2?.index !== "number") {
        return;
      }
      let num = result2.index + 1, judge = [];
      while (num--) {
        const judgeEvent = player2.judge();
        judgeEvent.judge2 = (result4) => result4.bool;
        judgeEvent.set("callback", async (event3) => {
          event3.getParent().orderingCards.remove(event3.card);
        });
        const result3 = await judgeEvent.forResult();
        if (!result3?.card) {
          break;
        } else {
          judge.push(result3.card);
        }
      }
      ...
      if (cards2.length) {
        ...
        while (cards2.length) {
          if (cards2.length > 1) {
            result3 = await player2.chooseButtonTarget({
              ...
              ai2(target2) {
                const { player: player3, given_map: map } = get.event(), att = get.attitude(player3, target2), card = ui.selected.buttons?.[0]?.link;
                if (!card) {
                  return 0;
                }
                if (get.value(card, player3, "raw") < 0) {
                  ...
                  const cards3 = map.has(target2) ? map.get(target2) : [];
                  return Math.max(0.1, att / Math.sqrt(1 + target2.countCards("h") + cards3.length));
                } else {
                  return 0;
                }
              },
            ...
            result3 = await player2.chooseTarget(`选择一名角色获得${get.translation(cards2)}`, true).set("ai", (target2) => {
              const { player: player3, given_map: map, toGive: card } = get.event(), att = get.attitude(player3, target2);
              if (!card) {
                return 0;
              }
              if (get.value(card, player3, "raw") < 0) {
          ...
          if (result3.bool) {
            ...
            const given = (given_map.get(target2) ?? []).concat(toGive);
            given_map.set(target2, given);
            if (!cards2.length) {
              break;
            }
          }
  ...
  clanquhuo: {
    ...
    filter(event2, player2) {
      ...
      return history.indexOf(event2) == 0 && game.hasPlayer((target2) => {
        return target2.isDamaged() && (target2.hasClan("弘农杨氏") || target2 == player2);
      });
    },
    async cost(event2, trigger2, player2) {
      const targets2 = game.filterPlayer((target2) => {
      ...
      if (targets2.length == 1) {
        const result2 = await player2.chooseBool(get.prompt2(event2.skill, targets2)).set("ai", () => {
          return get.attitude(get.player(), get.event().target) > 0;
        }).set("target", targets2[0]).forResult();
        if (result2.bool) {
          event2.result = {
            bool: true,
            targets: targets2
        ...
        event2.result = await player2.chooseTarget(get.prompt2(event2.skill), (card, player3, target2) => {
          return target2.isDamaged() && (target2.hasClan("弘农杨氏") || target2 == player3);
        }).set("ai", (target2) => {
          return get.recoverEffect(target2, get.player());
        }).forResult();
      }
    },
    async content(event2, trigger2, player2) {
  ...
  clanpoxi: {
    ...
    filterTarget(_, player2, target2) {
      if (target2.countDiscardableCards("he") <= 0) {
        return false;
      }
      return get.distance(player2, target2) <= 1;
    },
    mod: {
      globalFrom(from, to, current) {
    ...
    async content(event2, trigger2, player2) {
      const { target: target2 } = event2;
      const { cards: cards2 } = await player2.discardPlayerCard(target2, true).set("ai", function(button) {
    ...
    subSkill: {
      directHit: {
        ...
        async content(event2, trigger2, player2) {
          ...
          if (bool) {
            trigger2.directHit.addArray(trigger2.targets);
          }
        }
      },
      check: {
  ...
  clananran: {
    ...
    async cost(event2, trigger2, player2) {
      ...
      const result2 = await player2.chooseButton([
        ...
        return game.filterPlayer((target2) => get.effect(target2, { name: "draw" }, player3, player3) > 0).sort((a, b) => {
          return get.effect(b, { name: "draw" }, player3, player3) - get.effect(a, { name: "draw" }, player3, player3);
        }).slice(0, count2).reduce((sum, target2) => sum + get.effect(target2, { name: "draw" }, player3, player3), 0);
      }).forResult();
      event2.result = {
        bool: result2.bool,
    ...
    async content(event2, trigger2, player2) {
      ...
      if (cost_data == "draw") {
        ...
        const result2 = await player2.chooseTarget(`岸然：令至多${get.cnNumber(count)}名角色各摸一张牌`, [1, count], true).forResult();
        if (result2.targets?.length) {
          for (const i of result2.targets.sortBySeat()) {
  ...
  clangaobian: {
    ...
    filter(event2, player2) {
      ...
      const targets2 = game.filterPlayer2((target2) => target2.hasHistory("damage"));
      return targets2.length == 1 && targets2[0]?.isIn();
    },
    forced: true,
    logTarget(event2, player2) {
      return game.findPlayer2((target2) => target2.hasHistory("damage"));
    },
    async content(event2, trigger2, player2) {
      const target2 = game.findPlayer2((target3) => target3.hasHistory("damage"));
      const discarded = _status.discarded.filter((c) => c.name == "sha");
      const bool = discarded.some((c) => target2.hasUseTarget(c));
      const result2 = bool ? await target2.chooseButton(
        [
          "告变：请选择一项",
          [
            [
        ...
        "discarded",
        discarded.filter((c) => target2.hasUseTarget(c))
      ).set("ai", (button) => {
      ...
      if (result2.links[0] == "sha") {
        const result22 = await target2.chooseCardButton("告变：请选择其中一张【杀】使用", discarded, true).set("filterButton", (button) => {
          return get.player().hasUseTarget(button.link);
        }).forResult();
        if (result22?.bool && result22.links?.length) {
          await target2.chooseUseTarget(result22.links[0], true, false);
        }
      } else {
        await target2.loseHp();
      }
  ...
  clankaiji: {
    ...
    filterTarget(card, player2, target2) {
      return !get.event().clankaiji_enabledTargets.includes(target2) && player2.countDiscardableCards(target2, "h");
    },
    async content(event2, trigger2, player2) {
      const { target: target2 } = event2;
      const { links: cards2 } = await target2.discardPlayerCard(player2, "h", true).set("ai", (button) => {
        const { player: player3, target: target3 } = get.event(), { link } = button;
        if (get.attitude(player3, target3) > 0) {
          if (target3.hasUseTarget(link)) {
            return 10;
          }
          return 6 - get.value(link);
        }
        if (player3.hasSkillTag("viewHandcard", null, target3, true)) {
          if (!target3.hasUseTarget(link)) {
            return 10;
          }
          return get.value(link);
        }
        if (get.is.shownCard(link) && !target3.hasUseTarget(link)) {
          return 10;
        }
        return 1;
      }).forResult();
    ...
    ai: {
      ...
      result: {
        player(player2, target2) {
          if (!player2.hasCard((card) => player2.hasUseTarget(card), "h")) {
            return -1;
          }
          if (player2 == target2 || get.attitude(player2, target2) > 0) {
            return 1;
          }
          if (player2.countCards("h", (card) => player2.hasUseTarget(card)) >= player2.countCards("h", (card) => !player2.hasUseTarget(card))) {
  ...
  clanchengqi: {
      ...
      backup(links, player2) {
        return {
          ...
          ai1(card) {
            ...
            if (ui.selected.cards.length && game.hasPlayer((target2) => {
              return get.effect(target2, { name: "draw" }, player3, player3) > 0;
            })) {
    ...
    subSkill: {
      ...
      effect: {
        ...
        async cost(event2, trigger2, player2) {
          event2.result = await player2.chooseTarget("承启：是否令一名角色摸一张牌？").set("ai", (target2) => {
            const player3 = get.event().player;
  ...
  clanjieli: {
    ...
    async cost(event2, trigger2, player2) {
      ...
      event2.result = await player2.chooseTarget(get.prompt(event2.skill), "观看一名角色的牌名字数最多的手牌" + str, (card, player3, target2) => {
        return target2.countCards("h");
      }).set("ai", (target2) => {
        const player3 = get.event().player;
        const num2 = Math.max(...target2.getCards("h").map((card) => get.cardNameLength(card)));
        return num2 + 1e-4 * get.attitude(player3, target2);
      }).forResult();
    },
    async content(event2, trigger2, player2) {
      const target2 = event2.targets[0];
      const num = player2.getHistory("useCard").length > 0 ? Math.max(...player2.getHistory("useCard").map((history) => get.cardNameLength(history.card))) : 0;
      const limit = Math.max(...target2.getCards("h").map((card) => get.cardNameLength(card)));
      const cards2 = target2.getCards("h", (card) => get.cardNameLength(card) == limit);
      if (num > 0) {
        const topCards = get.cards(num);
        await game.cardsGotoOrdering(topCards);
        const result2 = await player2.chooseToMove("诫厉：交换其中任意张牌").set("list", [
          [get.translation(target2) + "牌名字数最多的手牌", cards2, "dcsushou_tag"],
          ["牌堆顶", topCards]
          ...
          const player3 = get.event().player, target3 = get.event().getParent().targets[0];
          const sgn = get.sgn(get.sgn(get.attitude(player3, target3)) - 0.5);
        ...
        if (result2.bool) {
          const lose = result2.moved[1].slice();
          const gain = result2.moved[0].slice().filter((i) => !get.owner(i));
        ...
        const content = ['###诫厉###<div class="text center">' + get.translation(target2) + "牌名字数最多的手牌</div>", cards2];
  ...
  clantanque: {
    ...
    locked: false,
    async cost(event2, trigger2, player2) {
      const evt = lib.skill.dcjianying.getLastUsed(player2, trigger2);
      const curCard = trigger2.card, prevCard = evt.card;
      const curNum = get.number(curCard), prevNum = get.number(prevCard);
      const delNum = Math.abs(curNum - prevNum);
      event2.result = await player2.chooseTarget(get.prompt(event2.skill), `对一名体力值或手牌数为${delNum}的角色造成1点伤害`, (card, player3, target2) => {
        return target2.getHp() === get.event().delNum || target2.countCards("h") == get.event().delNum;
      }).set("delNum", delNum).set("ai", (target2) => {
    ...
    init(player2) {
      ...
      if (history.length) {
        ...
        if (typeof get.number(trigger2.card, player2) != "number") {
          return;
        }
        player2.storage.clantanque_mark = trigger2.card;
  ...
  clanshengmo: {
    ...
    async content(event2, trigger2, player2) {
      ...
      const names = lib.inpile.filter((name2) => get.type(name2) == "basic" && !player2.getStorage("clanshengmo").includes(name2)), cards2 = evt.clanshengmo_cards.sort((a, b) => get.number(a, false) - get.number(b, false)), canChoose = cards2.filter((card) => !player2.getStorage("clanshengmo_num").includes(get.number(card, false)));
      ...
      for (const name2 of names) {
        const card = { name: name2, isCard: true };
  ...
  clanjianji: {
    getBool(event2, player2) {
      ...
      const targets2 = game.filterPlayer((target2) => {
        return event2.player.getPrevious() == target2 || event2.player.getNext() == target2;
      });
      const bool = !targets2.some((target2) => {
        return target2.getHistory("useCard").length;
      });
      const goon = player2.hasUseTarget(card) && !game.getGlobalHistory("useCard", (evt) => {
        return evt.targets?.some((target2) => targets2.includes(target2));
      }).length;
      return [bool, goon];
    },
    limited: true,
    ...
    async content(event2, trigger2, player2) {
      player2.awakenSkill(event2.name);
      const card = new lib.element.VCard({ name: "sha", isCard: true });
      const [boolx, goon] = get.info(event2.name).getBool(trigger2, player2);
      if (boolx) {
        let draw = false;
        if (goon) {
          const result2 = await player2.chooseBool("是否与" + get.translation(trigger2.player) + "各摸一张牌？").set("choice", get.attitude(player2, trigger2.player) > 0 || trigger2.player.countCards("h") > player2.countCards("h")).forResult();
          if (result2?.bool) {
            draw = true;
          }
        } else {
  ...
  clanqiajue: {
    ...
    async content(event2, trigger2, player2) {
      ...
      if (bool) {
        player2.when({
        ...
        }).filter((evt) => evt == trigger2).step(async (event3, trigger3, player3) => {
          ...
          if (num > 30) {
            ...
            player3.popup("洗具");
            const evt = trigger3.getParent("phase", true, true);
            if (evt?.phaseList) {
  ...
  clanbaichu: {
    ...
    forced: true,
    async content(event2, trigger2, player2) {
      const storage = player2.storage.clanbaichu || {}, suit = get.suit(trigger2.card);
      if (suit != "none") {
        const key = `${suit}+${get.type2(trigger2.card)}`;
        if (key in storage) {
          ...
          if (list.length) {
            ...
            const result2 = await player2.chooseButton(dialog, true).set("ai", function(button) {
            ...
            if (result2?.bool && result2?.links?.length) {
              const key2 = `${get.suit(trigger2.card)}+${get.type2(trigger2.card)}`, name = result2.links[0][2];
      ...
      if (Object.values(player2.getStorage("clanbaichu")).includes(trigger2.card.name)) {
  ...
  clanqiuxin: {
    ...
    content() {
      ...
      target.chooseControl().set("choiceList", [str + "下次对你使用【杀】后，其视为对你使用任意普通锦囊牌", str + "下次对你使用任意普通锦囊牌后，其视为对你使用【杀】"]).set("ai", function() {
        var target2 = _status.event.player;
        var player2 = _status.event.target;
        var num1 = get.effect(target2, get.autoViewAs({ name: "sha" }, []), player2, player2);
        ...
        for (var name of lib.inpile) {
          ...
          if (num2 < get.effect(target2, get.autoViewAs({ name }, []), player2, player2)) {
            num2 = get.effect(target2, get.autoViewAs({ name }, []), player2, player2);
          }
        }
    ...
    ai: {
      ...
      result: {
        target(player2, target2) {
          ...
          if (cards2.some((card) => player2.canUse(card, target2) && get.effect(target2, card, player2, player2) > 0)) {
      ...
      effect: {
        ...
        filter(event2, player2) {
          ...
          if (event2.card.name == "sha") {
            return event2.targets.some((target2) => {
              return player2.getStorage("clanqiuxin_effect").some((list) => list[0] == target2 && list[1] == 0);
            });
          }
          if (get.type(event2.card) == "trick") {
            return event2.targets.some((target2) => {
              return player2.getStorage("clanqiuxin_effect").some((list) => list[0] == target2 && list[1] == 1);
            });
          }
          return false;
        },
        ...
        content() {
          ...
          var targets2 = list.map((listx) => listx[0]);
          event.targets = targets2;
          var target2 = event.targets.shift();
          event.target = target2;
          var list = [];
          const nameFilter = trigger.card.name == "sha" ? (name2) => get.type(name2) == "trick" : (name2) => name2 == "sha";
          for (var name of lib.inpile) {
            ...
            if (!player.canUse(get.autoViewAs({ name }, []), target2)) {
              continue;
            }
          ...
          if (!list.length) {
            ...
            player.chooseButton(["求心：视为对" + get.translation(target2) + "使用一张牌", [list, "vcard"]], true).set("ai", function(button) {
          ...
          if (result.bool) {
            ...
            player.useCard(card, target2, false);
          }
  ...
  clanjianyuan: {
    ...
    content() {
      ...
      trigger.player.chooseCard("是否重铸任意张牌名字数为" + num + "的牌？", [1, Infinity], "he", (card, player2) => _status.event.cards.includes(card) && player2.canRecast(card), "allowChooseAll").set("ai", (card) => {
  ...
  clanjiejian: {
    ...
    content() {
      ...
      event.num = num;
      player.chooseTarget(get.prompt("clanjiejian"), "令一名目标角色摸" + get.cnNumber(num) + "张牌", function(card, player2, target3) {
        return _status.event.getTrigger().targets.includes(target3);
      }).set("ai", (target3) => get.attitude(_status.event.player, target3));
      if (result.bool) {
        var target2 = result.targets[0];
        player.logSkill("clanjiejian", target2);
        target2.draw(num);
      }
    },
    ai: {
      threaten: 3,
      effect: {
        player_use(card, player2, target2) {
          if (!target2 || typeof card !== "object" || player2._clanjiejian_mod_temp || get.type(card) === "equip" || get.attitude(player2, target2) <= 0 || get.cardNameLength(card) !== player2.getHistory("useCard").length + 1) {
            return;
          }
          let targets2 = [target2], evt = _status.event.getParent("useCard");
          targets2.addArray(ui.selected.targets);
          if (evt && evt.card == card) {
            targets2.addArray(evt.targets);
          }
          return [1, 0.8 * get.cardNameLength(card) / targets2.length];
        }
      }
    },
    mod: {
      aiOrder(player2, card, num) {
        if (typeof card == "object" && get.type(card) !== "equip") {
          ...
          let bool = game.hasPlayer((target2) => {
            if (get.attitude(player2, target2) <= 0 || !player2.canUse(card, target2, null, true)) {
              return false;
            }
            return get.effect(target2, card, player2, player2) + get.effect(target2, { name: "draw" }, player2, player2) > 0;
          });
  ...
  clanhuanghan: {
    ...
    async content(event2, trigger2, player2) {
      ...
      await player2.draw(get.cardNameLength(trigger2.card));
    ...
    ai: {
      ...
      effect: {
        target(card, player2, target2) {
          if (!lib.translate[card.name] || !get.tag(card, "damage") || player2.hasSkillTag("jueqing", false, target2)) {
            return;
          }
          let num = get.cardNameLength(card) - target2.getDamagedHp();
          if (num > 0) {
      ...
      switch (name) {
        case "roundStart": {
          ...
          if (result2?.bool && result2.cards?.length) {
            await player2.showCards(result2.cards, get.translation(player2) + "发动了【迂志】");
            player2.addGaintag(result2.cards, "clanyuzhi");
            await player2.draw(get.cardNameLength(result2.cards[0]));
            player2.storage.clanyuzhi_mark = get.cardNameLength(result2.cards[0]);
        ...
        case "roundEnd": {
          ...
          if (num1 > 0 && num2 > 0 && num1 > num2 || num1 > num3) {
            ...
            if (player2.hasSkill("clanbaozu", null, false, false)) {
              result2 = await player2.chooseBool("迂志：是否失去〖保族〗？", "若选择“否”，则你受到1点雷属性伤害").set("choice", player2.awakenedSkills.includes("clanbaozu")).forResult();
            } else {
              result2 = { bool: false };
            }
            if (result2?.bool) {
  ...
  clanxieshu: {
    ...
    async cost(event2, trigger2, player2) {
      var num = get.cardNameLength(trigger2.card), str = "";
  ...
  clanfuxun: {
    ...
    filterTarget(card, player2, target2) {
      ...
      if (!ui.selected.cards.length) {
        return target2.countGainableCards(player2, "h") > 0;
      }
      return true;
    },
    check(card) {
      ...
      var targets2 = game.filterPlayer((target3) => target3 != player2 && lib.skill.clanfuxun.ai.result.target(player2, target3) != 0);
      targets2.sort((a, b) => Math.abs(lib.skill.clanfuxun.ai.result.target(player2, b)) - Math.abs(lib.skill.clanfuxun.ai.result.target(player2, a)));
      if (evtx && targets2.length) {
    ...
    ai: {
      ...
      result: {
        target(player2, target2) {
          var evtx = _status.event.getParent("phaseUse");
          var num = get.sgn(get.attitude(player2, target2));
  ...
  clanjiexuan: {
    ...
    check(card) {
      ...
      var storage = player2.storage.clanjiexuan;
      var name = (storage || 0) % 2 ? "guohe" : "shunshou";
      var fix = player2.hasSkill("clanzhongliu") && get.position(card) != "h" ? 2 : 1;
  ...
  clanmingjie: {
    ...
    limited: true,
    filterTarget(card, player2, target2) {
      return !Object.keys(target2.storage).some((skill) => {
        return skill.startsWith("clanmingjiex_" + player2.playerid + "_") && target2.storage[skill][0] === 1 + (_status.currentPhase === target2);
      });
    },
    skillAnimation: true,
    animationColor: "thunder",
        ...
        target(player2, target2) {
          if (player2.hasSkill("clanzhongliu") || player2.hp == 1) {
            if (!player2.hasCard((card) => {
              ...
              if (!lib.filter.targetEnabled2(card, player2, target2)) {
                return false;
              }
              return game.hasPlayer((current) => {
                return player2.canUse(card, current) && get.effect(current, card, player2, player2) > 0 && current != target2 && get.effect(target2, card, player2, player2) > 0;
              });
            }, "hs")) {
              return 0;
            }
          ...
          return get.sgnAttitude(player2, target2);
        }
      }
    },
    subSkill: {
      effect: {
        ...
        async cost(event2, trigger2, player2) {
          event2.result = await player2.chooseTarget(
            get.prompt(event2.skill),
            "令任意【铭戒】目标角色成为" + get.translation(trigger2.card) + "的目标",
            (card, player3, target2) => {
              const trigger3 = get.event().getTrigger();
              if (trigger3.targets.includes(target2) || !Object.keys(target2.storage).some((skill) => skill.startsWith("clanmingjiex_" + player3.playerid + "_"))) {
                return false;
              }
      ...
      targeted: {
        ...
        async content(event2, trigger2, player2) {
          ...
          while (cards2.some((card) => get.position(card, true) == "d" && player2.hasUseTarget(card))) {
            const result2 = await player2.chooseButton(["铭戒：是否使用这些牌？", cards2]).set("filterButton", (button) => {
              return get.player().hasUseTarget(button.link);
            }).set("ai", (button) => {
              return get.player().getUseValue(button.link);
            }).forResult();
        ...
        return {
          ...
          async content(event2, trigger2, player3) {
            ...
            if (!targets2?.length) {
              ...
              const result3 = await player3.choosePlayerCard(target2, "h", true, [1, 4]).set("prompt", "观骨：观看" + get.translation(target2) + "的至多四张牌").set("ai", (button) => {
                if (ui.selected.buttons.length >= _status.event.num) {
                  return 0;
                }
                return Math.random();
              }).set("num", ret).forResult();
              cards2 = result3.cards;
            }
            if (cards2?.length) {
              const count = cards2.length;
              event2.getParent().viewedCount = count;
              const result3 = await player3.chooseButton(["观骨：是否使用其中一张牌？", cards2]).set("filterButton", (button) => {
          ...
          ai: {
            ...
            result: {
              target(player3, target2) {
                return -Math.min(target2.countCards("h"), 4) / 2;
              }
            }
          }
        };
      ...
      if (!num || target.countCards("he") < num) {
        ...
        target.chooseCard(get.translation(player) + "对你发动了【驳龙】", str, num, "he").set("ai", (card) => {
          ...
          (function() {
            ...
            if (game.hasPlayer((current) => {
              return player.canUse(sha2, current, true, true) && get.effect(current, sha2, player, target) < 0 && !current.countCards("hs", ["shan", "caochuan"]);
            })) {
              return false;
            }
            return true;
  ...
  clanlianzhu: {
    ...
    subSkill: {
      global: {
        ...
        filterTarget(card, player2, target2) {
          return target2.hasSkill("clanlianzhu") && !target2.hasSkill("clanlianzhu_targeted") && (!target2.storage.clanlianzhu || target2.storage.clanlianzhu && game.hasPlayer((current) => {
            if (current == player2 || current == target2) {
              return false;
            }
            return current.inRangeOf(player2) || current.inRangeOf(target2);
          }));
        },
        selectTarget() {
          var player2 = _status.event.player;
        ...
        filterOk() {
          ...
          if (!target2.storage.clanlianzhu) {
        ...
        prompt() {
          ...
          game.countPlayer(function(target2) {
            if (target2.hasSkill("clanlianzhu") && !target2.hasSkill("clanlianzhu_targeted")) {
              if (target2.storage.clanlianzhu) {
                if (game.hasPlayer((current) => {
                  if (current == player2 || current == target2) {
                    return false;
        ...
        content() {
          ...
          event.finish();
          target.chooseTarget("联诛：选择其与你使用【杀】的目标", true, (card, player2, target2) => {
            if (target2 == player2 || target2 == _status.event.sourcex) {
              return false;
            }
            return target2.inRangeOf(player2) || target2.inRangeOf(_status.event.sourcex);
          }).set("ai", (target2) => {
        ...
        ai: {
          ...
          result: {
            player(player2, target2) {
              if (!target2.storage.clanlianzhu && player2.hasCard((card) => get.value(card) < 5, "he")) {
                return 1;
              }
              return 0;
            },
            target(player2, target2) {
              if (target2.storage.clanlianzhu && player2.hasSha()) {
                return 1;
              }
              return 0;
            }
  ...
  clanfangzhen: {
    ...
    content() {
      "step 0";
      player.chooseTarget(get.prompt2("clanfangzhen"), (card, player2, target3) => {
        return !target3.isLinked();
      }).set("ai", (target3) => {
        var player2 = _status.event.player;
        if (_status.event.goon && target3 != player2) {
          target3.classList.add("linked");
          target3.classList.add("linked2");
          try {
            var cards2 = player2.getCards("hs", (cardx) => {
              if (get.name(cardx) != "sha") {
                return false;
              }
              return game.hasNature(cardx, "linked");
            });
            cards2.map((i) => [i, get.effect(target3, i, player2, player2)]);
            cards2.sort((a, b) => b[1] - a[1]);
          } catch (e) {
            target3.classList.remove("linked");
            target3.classList.remove("linked2");
          }
          target3.classList.remove("linked");
          target3.classList.remove("linked2");
          var eff = cards2[0][1];
          ...
          return Math.max(2 * get.effect(target3, { name: "draw" }, player2, player2) + 0.6 * get.effect(player2, { name: "draw" }, player2, player2), get.recoverEffect(target3, player2, player2));
        }
        return Math.max(2 * get.effect(target3, { name: "draw" }, player2, player2) + 0.6 * get.effect(player2, { name: "draw" }, player2, player2), get.recoverEffect(target3, player2, player2));
      }).set(
        "goon",
      ...
      if (result.bool) {
        var target2 = result.targets[0];
        event.target = target2;
        player.logSkill("clanfangzhen", target2);
        player.addSkill("clanfangzhen_remove");
        player.markAuto("clanfangzhen_remove", [target2.getSeatNum()]);
        target2.link(true);
        var choices = ["选项一"], choiceList = ["摸两张牌，然后交给" + get.translation(target2) + "两张牌", "令" + get.translation(target2) + "回复1点体力"];
        ...
        player.chooseControl(choices).set("prompt", "放赈：请选择一项").set("choiceList", choiceList).set("ai", () => {
          ...
          if (get.attitude(player2, target3) <= 0 && player2.countCards("he", (card) => get.value(card) < 0) >= 2) {
            return 0;
          }
          return 2 * get.effect(target3, { name: "draw" }, player2, player2) + 0.6 * get.effect(player2, { name: "draw" }, player2, player2) > get.recoverEffect(target3, player2, player2) ? 0 : 1;
        });
      } else {
        event.finish();
      }
      ...
      if (!player.countCards("he")) {
        ...
        player.chooseCard("放赈：交给" + get.translation(target2) + "两张牌", "he", 2, true);
      }
      if (result.bool) {
  ...
  clanliuju: {
    ...
    async cost(event2, trigger2, player2) {
      event2.result = await player2.chooseTarget(get.prompt(event2.skill), "与一名其他角色拼点，输的角色可以使用任意张拼点牌中的非基本牌", (card, player3, target2) => {
        return player3.canCompare(target2);
      }).set("ai", (target2) => {
        var player3 = _status.event.player;
    ...
    content() {
      ...
      event.target = target2;
      player.chooseToCompare(target2).set("small", true);
      if (!result.tie) {
        var loser = result.bool ? target2 : player;
        var cards2 = [];
        ...
        event.loser = loser;
        event.distance = [get.distance(player, target2), get.distance(target2, player)];
        if (cards2.length) {
          event.cards = cards2;
        } else {
          event.finish();
      ...
      var cardsx = cards2.filter((i) => get.position(i, true) == "d" && event.loser.hasUseTarget(i));
      if (!cardsx.length) {
        event.goto(6);
      } else {
        event.loser.chooseButton(["留驹：是否使用其中的一张牌？", cardsx]).set("filterButton", (button) => {
      ...
      if (cards2.filter((i) => get.position(i, true) == "d" && event.loser.hasUseTarget(i)).length) {
        event.goto(2);
      }
      if (get.distance(player, target2) != event.distance[0] || get.distance(target2, player) != event.distance[1]) {
  ...
  clanxumin: {
    ...
    filterTarget(card, player2, target2) {
      ...
      return player2.canUse(card, target2);
    },
  ...
  clanlianhe: {
    ...
    content() {
      "step 0";
      player.chooseTarget(get.prompt2("clanlianhe"), 2, (card, player2, target2) => {
      ...
      if (result.bool) {
        ...
        targets2.forEach((i) => i.link(true));
        player.logSkill("clanlianhe", targets2);
        player.addSkill("clanlianhe_effect");
        player.markAuto("clanlianhe_effect", targets2);
      }
    },
    subSkill: {
      effect: {
  ...
  clanhuanjia: {
    ...
    content() {
      "step 0";
      player.chooseTarget(get.prompt("clanhuanjia"), "与一名其他角色拼点，赢的角色可以使用一张拼点牌。若此牌未造成过伤害，你获得另一张拼点牌，否则你失去一个技能", (card2, player2, target3) => {
        ...
        if (get.attitude(player2, target3) <= 0) {
          ...
          var ts = target3.getCards("h").sort((a, b) => get.number(b) - get.number(a));
          if (!hs.length || !ts.length) {
            return 0;
          }
      ...
      if (result.bool) {
        var target2 = result.targets[0];
        event.target = target2;
        player.logSkill("clanhuanjia", target2);
        player.chooseToCompare(target2);
      } else {
        event.finish();
      }
      if (!result.tie) {
        var winner = result.bool ? player : target2;
        var cards2 = [];
        game.getGlobalHistory("cardMove", (evt) => {
      ...
      var cardsx = cards2.filter((i) => get.position(i, true) == "d" && event.winner.hasUseTarget(i));
      if (!cardsx.length) {
        event.goto(6);
      } else {
        event.winner.chooseButton(["缓颊：是否使用其中的一张牌？", cardsx]).set("filterButton", (button) => {
          return _status.event.player.hasUseTarget(button.link);
        }).set("ai", (button) => {
          var damage = 1;
      ...
      if (game.hasPlayer2((current) => {
        ...
        var skills2 = player.getSkills(null, false, false).filter((skill) => {
          var info = get.info(skill);
        ...
        player.chooseControl(skills2).set(
          "choiceList",
          skills2.map((i) => {
    ...
    async cost(event2, trigger2, player2) {
      const func = (event3, player3) => {
        ...
        game.countPlayer((target2) => {
          if (target2 != player3) {
            target2.prompt(get.translation(name) + get.info(name).getNum(target2));
          }
        });
      };
      ...
      const cards2 = trigger2.cards.filterInD("oe");
      event2.result = await player2.chooseTarget("三恇：选择一名其他角色", "令其交给你至少X张牌" + (cards2.length ? "，然后其获得" + get.translation(cards2) : "") + "（X为以下条件中其满足的项数：场上有牌、已受伤、体力值小于手牌数）", true, lib.filter.notMe).set("ai", (target2) => {
        const { player: player3, goon } = get.event();
        const att = get.attitude(player3, target2), num = lib.skill.clansankuang.getNum(target2);
        if (num == 0) {
          return att;
        }
        if (goon) {
        ...
        return -Math.sqrt(Math.abs(att)) - lib.skill.clansankuang.getNum(target2);
      }).set(
        "goon",
        Math.max.apply(
          Math,
          trigger2.cards.map((i) => get.value(i))
        ) <= 5 || trigger2.cards.filterInD("oe").length == 0
      ).forResult();
    },
    async content(event2, trigger2, player2) {
      const {
      ...
      const num = lib.skill.clansankuang.getNum(target2), num2 = target2.countCards("he");
      const cards2 = trigger2.cards.filterInD("oe");
      const result2 = num2 == 0 ? { bool: false } : await target2.chooseToGive(player2, "he", num > 0, [Math.min(num2, num), Infinity], "allowChooseAll").set("ai", get.unuseful).set("prompt", num > 0 ? "是否交给" + get.translation(player2) + "任意张牌" + (cards2.length ? "并获得" + get.translation(cards2) : "") + "？" : "交给" + get.translation(player2) + "至少" + get.cnNumber(num) + "张牌").forResult();
      if (!result2?.bool || !result2.cards?.length || !cards2.length) {
        return;
      }
      await game.delayx();
      if (trigger2.cards.filterInD().length) {
        await target2.gain(trigger2.cards.filterInD(), "gain2", "bySelf");
      } else if (trigger2.cards.filterInD("e").length) {
        await target2.gain(trigger2.cards.filterInD("e"), get.owner(trigger2.cards.filterInD("e")[0]), "give");
      }
    },
    ai: {
      reverseOrder: true,
      ...
      effect: {
        target_use(card, player2, target2) {
          if (player2 == target2 && get.type(card) == "equip" && !player2.getHistory("useCard", (evt) => get.type(evt.card) == "equip").length == 0) {
            return [1, 3];
          }
        }
      },
  ...
  clanbeishi: {
    ...
    filter(event2, player2) {
      ...
      const target2 = history[0].targets[0];
  ...
  clanshenjun: {
    ...
    content() {
      var cards2 = player.getCards("h", trigger.card.name);
      player.showCards(cards2, get.translation(player) + "发动了【神君】");
    ...
    subSkill: {
      viewAs: {
        ...
        content() {
          "step 0";
          var cards2 = player.getCards("h", (card2) => card2.hasGaintag("clanshenjun"));
          var list = [], names = [];
          ...
          player.chooseButton(["是否将" + get.cnNumber(cards2.length) + "张牌当下列一张牌使用？", [list, "vcard"]]).set("ai", function(button) {
            return _status.event.player.getUseValue({
              name: button.link[2],
              nature: button.link[3]
            });
          ...
          if (result.bool) {
            ...
            game.broadcastAll(
              function(num, card2) {
                ...
                lib.skill.clanshenjun_backup.viewAs = card2;
              },
              cards2.length,
              { name, nature }
            );
  ...
  clanyunshen: {
    ...
    content() {
      ...
      player.chooseControl().set("choiceList", [name + "视为对你使用一张冰【杀】", "你视为对" + name + "使用一张冰【杀】"]).set("prompt", "熨身：请选择一项").set("ai", () => _status.event.choice).set(
        "choice",
        (function() {
          var card2 = {
            name: "sha",
          ...
          var eff = get.effect(player, card2, target, player), eff2 = get.effect(target, card2, player, player);
          if (eff > eff2) {
            return "选项一";
          } else {
            return "选项二";
  ...
  clanfenchai: {
    ...
    init(player2) {
      ...
      var history = player2.getHistory("useSkill", (evt2) => {
        ...
        var skill = get.sourceSkillFor(evt2), targets3 = evt2.targets;
        var info = get.info(skill);
        if (!info || info.charlotte) {
          return false;
        }
        if (targets3 && targets3.length) {
          if (targets3.filter((i) => player2.differentSexFrom(i)).length > 0) {
            return true;
          }
        }
        return false;
      ...
      if (history.length) {
        var evt = history[0], targets2 = evt.targets;
        player2.markAuto(
          "clanfenchai",
          targets2.filter((i) => player2.differentSexFrom(i))
        );
      }
    },
    trigger: {
    ...
    filter(event2, player2) {
      ...
      return targets2.filter((i) => player2.differentSexFrom(i)).length > 0;
    },
    content() {
      player.markAuto(
        "clanfenchai",
      ...
      backupx: {
        ...
        content() {
          ...
          if (choice == "damage") {
            ...
            var cards2 = player.getCards("h", choice);
            if (cards2.length) {
          ...
          if (!player.isIn() || !game.hasPlayer((current) => current != player)) {
            ...
            player.chooseTarget("烈誓：令一名其他角色选择另一项", lib.filter.notMe, true).set("ai", (target3) => {
              var player2 = _status.event.player, chosen = _status.event.getParent().choice, att = get.attitude(player2, target3);
              if (chosen == "damage") {
                if (att > 0) {
                  return 0;
                }
                return -att / 2 + target3.countCards("h", (card) => ["sha", "shan"].includes(get.name(card)));
              }
              return get.damageEffect(target3, player2, player2, "fire");
            });
          }
          if (result.bool) {
            var target2 = result.targets[0];
            ...
            choiceList = choiceList.map((link, ind, arr) => {
              ...
              if (ind == 0) {
                if (target2.isDisabledJudge()) {
                  ok = false;
                }
              } else if (ind > 0) {
                var name = ind == 1 ? "shan" : "sha";
                if (!target2.countCards("h", name)) {
                  ok = false;
                }
              }
              if (!ok) {
            ...
            if (!list.length) {
              game.log(target2, "没有能执行的选项");
              event.finish();
              return;
            }
            target2.chooseControl(list).set("choiceList", choiceList).set("ai", () => {
              var controls = _status.event.controls.slice(), player2 = _status.event.player, user = _status.event.getParent().player;
          ...
          if (result.control == "选项一") {
            if (!target2.isDisabledJudge()) {
              target2.disableJudge();
            }
            target2.damage("fire");
          } else {
            var cards2 = target2.getCards("h", result.control == "选项二" ? "shan" : "sha");
            if (cards2.length) {
  ...
  clandianzhan: {
    ...
    content() {
      ...
      var cards2 = player.getCards("h", (card) => get.suit(card) == get.suit(trigger.card) && player.canRecast(card));
      if (cards2.length > 0) {
  ...
  clandaojie: {
    ...
    async content(event2, trigger2, player2) {
      ...
      if (!skills2.length) {
        ...
        result2 = await player2.chooseButton(["蹈节：失去一个锁定技，或点“取消”失去1点体力", [skills2, "skill"]]).set("displayIndex", false).set("ai", (button) => {
          const player3 = get.player();
      ...
      const targets2 = game.filterPlayer((current) => current == player2 || current.hasClan("颍川荀氏"));
      if (!targets2.length || !trigger2.cards.someInD()) {
        return;
      }
      result2 = targets2.length == 1 ? { bool: true, targets: targets2 } : await player2.chooseTarget("蹈节：将" + get.translation(trigger2.cards.filterInD()) + "交给一名颍川荀氏角色", true, (card, player3, target2) => {
        return target2 == player3 || target2.hasClan("颍川荀氏");
      }).set("ai", (target2) => get.attitude(get.player(), target2)).forResult();
      if (result2?.bool && result2?.targets?.length) {
        const target2 = result2.targets[0];
        player2.line(target2, "green");
        if (trigger2.cards.someInD()) {
          await target2.gain(trigger2.cards.filterInD(), player2, "gain2");
        }
      }
    }
  },
  ...
  clanmuyin: {
    ...
    content() {
      "step 0";
      player.chooseTarget(get.prompt("clanmuyin"), "令一名陈留吴氏角色的手牌上限+1", function(card, player2, current) {
        return (current == player2 || current.hasClan("陈留吴氏")) && !lib.skill.clanmuyin.isMax(current);
      ...
      if (result.bool) {
        var target2 = result.targets[0];
        player.logSkill("clanmuyin", target2);
        lib.skill.chenliuwushi.change(target2, 1);
        game.delayx();
      }
    }
  },
