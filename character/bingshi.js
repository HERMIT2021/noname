...
const skills = {
  ...
  pottuntian: {
    ...
    filter(event2, player2) {
      ...
      return player2.countCharge(true) && event2.getl?.(player2)?.cards2.some((card) => !get.is.damageCard(card));
    },
    ...
    subSkill: {
      phaseUse: {
        ...
        async precontent(event2, trigger2, player2) {
          ...
          const result = await player2.chooseNumbers(`###${get.translation(skill)}###出牌阶段限一次，你可以消耗任意点蓄力点，令至多等量名角色从牌堆或弃牌堆中各获得一张红桃牌`, [{ prompt: "请选择要移去的蓄力值", min: 1, max: player2.countCharge() }]).set("processAI", () => {
  ...
  potjixi: {
    ...
    filter(event2, player2) {
      return event2.markName == "charge" && event2.num >= 3;
    },
    async content(event2, trigger2, player2) {
      const { num } = trigger2;
  ...
  potzaoxian: {
    ...
    filter(event2, player2) {
      return get.info("potzaoxian").getTargets(player2).length && _status.currentPhase?.countDiscardableCards(player2, "he") > 0;
    },
    getTargets(player2) {
    ...
    check(event2, player2) {
      return get.info("potzaoxian").getTargets(player2).some((target) => get.effect(target, { name: "shunshou" }, player2, player2) > 0);
    },
    async content(event2, trigger2, player2) {
      const {
        targets: [target]
      ...
      if (targets.length) {
        const result = await player2.chooseTarget(`凿险：视为对任意名其他角色使用一张无距离限制的【顺手牵羊】`, [1, Infinity], (card2, player3, target2) => {
          return get.event().targets.includes(target2);
        }).set("_get_card", card).set("targets", targets).set("ai", (target2) => get.effect(target2, get.card(), get.player(), get.player())).forResult();
        const { targets: targetsx } = result;
        if (targetsx?.length) {
          await player2.useCard(card, targetsx);
        }
      }
    }
  },
  ...
  mbxiezhi: {
    ...
    async content(event2, trigger2, player2) {
      ...
      const num = Math.min(Math.abs(trigger2.num), max);
      if (num > 0) {
        player2.addCharge(num);
      }
      const num2 = Math.abs(trigger2.num) - num;
      if (num2 > 0) {
  ...
  potceduan: {
    ...
    async content(event2, trigger2, player2) {
      ...
      await player2.showCards(cards2, `${get.translation(player2)}发动了【策断】`).set("showers", targets);
  ...
  potzhengshuo: {
    ...
    getAIScore(player2) {
      let score = 0, enemyGift = 0;
      game.countPlayer((target) => {
        const attitude = get.attitude(player2, target), hand = target.countCards("h"), change = 4 - hand;
    ...
    ai: {
      order(item, player2) {
        return lib.skill.potzhengshuo.getAIScore(player2) > 0 ? 10 : 0;
      },
      threaten: 10086,
      result: {
        player(player2) {
          return lib.skill.potzhengshuo.getAIScore(player2);
        }
      }
    }
  },
  ...
  mbxiongtu: {
    ...
    async content(event2, trigger2, player2) {
      ...
      if (result?.cards?.length) {
        ...
        const resultx = await player2.chooseToDiscard(`凶图：取消并弃置${get.translation(card)}或弃置${num}张牌对${get.translation(target)}造成1点伤害`, "he", [0, Infinity]).set("filterOk", () => {
  ...
  potyinzhan: {
    ...
    filter(event2, player2) {
      ...
      if (player2.hp <= target.hp || player2.countCards("he") <= target.countCards("he")) {
        return true;
      }
      return false;
    },
    ...
    async content(event2, trigger2, player2) {
      const target = trigger2.player, bool1 = target.hp >= player2.hp, bool2 = target.countCards("he") >= player2.countCards("he");
  ...
  mbrunwei: {
    ...
    subSkill: {
      twice: {
        ...
        filter(event2, player2) {
          return event2.getl(player2)?.cards2?.length && player2.hasMark("mbrunwei_twice");
        },
        silent: true,
        content() {
    ...
    subSkill: {
      mark: {
        ...
        async content(event2, trigger2, player2) {
          ...
          player2.addMark("pothongyi", Math.min(trigger2.name === "damage" ? 1 : 2, num));
        }
      }
    }
  },
      ...
      lie: {
        ...
        content() {
          ...
          player.chooseUseTarget("###" + get.prompt("potzhanlie") + '###<div class="text center">' + str + "视为使用一张无次数限制的【杀】</div>", new lib.element.VCard({ name: "sha", isCard: true }), false).set("oncard", () => {
            const event2 = get.event(), { player: player2 } = event2, num = player2.countMark("potzhanlie_lie");
            player2.addTempSkill("potzhanlie_buff");
            player2.clearMark("potzhanlie_lie");
            event2.set("potzhanlie", Math.floor(num / 2));
          }).logSkill = "potzhanlie";
        },
        marktext: "烈",
        intro: {
