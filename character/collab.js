...
const skills = {
    ...
    async content(event2, trigger2, player2) {
      ...
      if (result?.bool && result?.targets?.length) {
        ...
        const control = await player2.chooseControl("手牌区", "装备区", "判定区", true).set("ai", function() {
          const target3 = get.event().target;
          if (target3.countCards("h") > target3.countCards("e")) {
            return 0;
          }
          return 1;
        }).set("target", target2).set("prompt", `请选择${get.translation(target2)}的一个区域`).forResult();
        const choice = {
          手牌区: "h",
          装备区: "e",
          判定区: "j"
        }[control.control];
        if (target2.countCards(choice, (card2) => get.type(card2) == "equip")) {
          await player2.gain(target2.getGainableCards(player2, choice), target2, "giveAuto");
        } else {
          target2.chat("沒有！没有！没有！");
        }
      }
    }
  },
    ...
    subSkill: {
      update: {
        ...
        filter(event2, player2) {
          ...
          if (event2?.gaintag?.includes(skill)) {
            return event2.name == "loseAsync" ? event2.type == "addToExpansion" : event2.name == "addToExpansion";
          }
  ...
  mbjimi: {
    ...
    async content(event2, trigger2, player2) {
      ...
      targets.forEach(
        (target2) => map.set(
          target2,
          target2.getCards("h", (card2) => !["tao", "jiu"].includes(get.name(card2)))
        )
      );
      ...
      await game.doAsyncInOrder(
        ...
        async (target2) => {
          ...
          if (list[0].length) {
            await game.cardsGotoPile(list[0], (event3, card2) => event3.list[1][event3.list[0].indexOf(card2)]).set("list", list);
          }
          if (cards2.length) {
            target2._start_cards = target2.getCards("h").concat(cards2);
            return target2.gain(cards2, "draw").set("delay", false);
          }
        },
        () => false
      );
    ...
    subSkill: {
      gain: {
        ...
        filter(event2, player2) {
          ...
          return event2.getd?.().some((card2) => ["tao", "jiu"].includes(get.name(card2)));
        },
        async content(event2, trigger2, player2) {
  ...
  mbmaodie: {
    ...
    filter(event2, player2) {
      ...
      return player2.countMark(`mbmaodie_used`) < 2 && get.info("mbmaodie").getCards(player2, event2.targets || []).length > 0;
    },
    getCards(player2, targets) {
      return targets.flatMap(
        (target2) => (target2._start_cards || []).filter((card2) => "cdhej".includes(get.position(card2)) && get.owner(card2) !== player2)
      );
    },
    async content(event2, trigger2, player2) {
      if (player2.hasHistory("sourceDamage", (evt) => evt.card == trigger2.card)) {
        player2.addTempSkill(`${event2.name}_limit`);
        player2.setStorage(`${event2.name}_limit`, get.cardNameLength(trigger2.card), true);
      } else {
        player2.addTempSkill(`${event2.name}_used`);
        player2.addMark(`${event2.name}_used`, 1, false);
        const card2 = get.info(event2.name).getCards(player2, trigger2.targets).randomGet();
        if (card2) {
          let animate = ["gain2"];
          if (get.owner(card2)) {
  ...
  dmchongqi: {
    ...
    usable: 1,
    filter(event2, player2) {
      return event2.card.name == "sha" && event2.target.countDiscardableCards(player2, "he");
    },
    check(event2, player2) {
      return get.effect(event2.target, { name: "guohe_copy2" }, player2, player2) > 0;
    },
    logTarget: "target",
    async content(event2, trigger2, player2) {
      ...
      if (player2.inRange(target2) && !target2.inRange(player2)) {
  ...
  dmfanquan: {
    ...
    async cost(event2, trigger2, player2) {
      event2.result = await player2.chooseTarget(get.prompt2(event2.skill), lib.filter.notMe).set("ai", (target2) => {
        const player3 = get.player();
        const eff = get.damageEffect(target2, player3, player3);
        if (player3.inRange(target2) && target2.inRange(player3)) {
          return eff * 2;
        }
        return eff;
      }).forResult();
    ...
    async content(event2, trigger2, player2) {
      ...
      if (player2.inRange(target2) && target2.inRange(player2)) {
        ...
        if (trigger2.num > 0) {
          await target2.damage(trigger2.num);
        }
  ...
  bachiqionggouyu_skill: {
    ...
    filter(event2, player2) {
      ...
      if (event2.name == "phaseUSe") {
        ...
        return !damageed && !event2.numFixed;
      }
    },
    async content(event2, trigger2, player2) {
  ...
  bazhijing_skill: {
    ...
    filter(event2, player2, triggername) {
      ...
      const bool = player2.getStorage("bazhijing").includes(event2.card.name);
    ...
    async content(event2, trigger2, player2) {
      if (event2.triggername == "damageAfter") {
        player2.markAuto("bazhijing", trigger2.card.name);
      } else {
        trigger2.cancel();
      }
    }
  ...
  olmojin: {
    ...
    init(player2, skill) {
      const mojinMap = [
        [
          ...
          { player: ["useCard"] },
          (evt, player3) => {
            const history = player3.getAllHistory(
              "useCard",
              (evt2) => get.type(evt2.card, null, false) != "basic" && !player3.getStorage("immojin").includes(evt2)
            );
        ...
        [
          ...
          (evt, player3) => {
            const history = player3.getAllHistory(
              "useCard",
              (evt2) => get.type(evt2.card, null, false) == "equip" && !player3.getStorage("immojin").includes(evt2)
            );
        ...
        [
          ...
          (evt, player3) => {
            const history = player3.getAllHistory(
              "useCard",
              (evt2) => get.type(evt2.card, "trick", false) == "trick" && !player3.getStorage("immojin").includes(evt2)
            );
            return history.length >= 2;
          },
          (player3) => {
        ...
        [
          ...
          (evt, player3) => {
            const history = player3.getAllHistory(
              "useCard",
              (evt2) => get.type(evt2.card, null, false) == "delay" && !player3.getStorage("immojin").includes(evt2)
            );
            return history.length >= 1;
          },
          (player3) => {
    ...
    async content(event2, trigger2, player2) {
      ...
      player2.when(data[1]).filter(data[2]).step(async (event3, trigger3, player3) => {
        if (data[3]) {
    ...
    subSkill: {
      ...
      effect: {
        ...
        filter(event2, player2) {
          const useCard = event2.getParent("useCard", true, true);
  ...
  oltuoquan: {
    ...
    async cost(event2, trigger2, player2) {
      ...
      } else if (player2.getStorage(event2.skill).length == 1) {
        event2.result.cost_data = true;
      }
    },
    ...
    async content(event2, trigger2, player2) {
      if (trigger2.name != "phaseZhunbei") {
        const func = async (target2) => {
          if (!target2.hasSkill("oldianzan", null, null, false)) {
            await target2.addSkills("oldianzan");
          }
          target2.markAuto("oldianzan", player2);
        };
        await game.doAsyncInOrder(event2.targets, func);
        return;
      }
      const nows = player2.getStorage(`${event2.name}_current2`);
      if (nows.length) {
        player2.setStorage(`${event2.name}_current2`, null);
        await player2.addAdditionalSkills(event2.name, []);
        game.broadcastAll(
          (player3, names2) => {
      ...
      if (!player2.getStorage(event2.name).length && !player2._gainJiangFei) {
        player2._gainJiangFei = true;
        const jiangfei = Math.random() > 0.99 ? ["jiangfei"] : ["ol_jiangwan", "ol_feiyi"];
        player2.markAuto(event2.name, jiangfei);
      }
      const names = player2.getStorage(event2.name).randomGets(4);
      if (!names.length) {
      ...
      if (result?.bool && result.links?.length) {
        ...
        await player2.addAdditionalSkills(event2.name, skills2);
        game.broadcastAll(
          (player3, names2) => {
        ...
        player2.setStorage(`${event2.name}_current`, [...fuchens]);
        player2.setStorage(`${event2.name}_current2`, [...fuchens]);
  ...
  oldianzan: {
    clickableFilter(player2) {
      ...
      return targets.some((target2) => target2?.isIn());
    },
    init(player2, skill) {
      ...
      if (!_status._click_throwFlower) {
        game.broadcastAll(() => {
          _status._click_throwFlower = function() {
            const target2 = this, player3 = game.me;
            if (!player3?._click_throwFlower?.includes(target2)) {
              return;
            }
            player3._click_throwFlower = [];
            if (game.online) {
              game.requestSkillData("oldianzan", "throwEmotion", 5e3, target2);
            } else {
              player3.throwEmotion(target2, ["flower", "wine", "egg", "shoe"].randomGet());
            }
          };
    ...
    sync: {
      throwEmotion(player2, target2) {
        player2.throwEmotion(target2, ["flower", "wine", "egg", "shoe"].randomGet());
        return;
      }
    },
    clickable(player2) {
  ...
  olxianglv: {
    ...
    filter(event2, player2) {
      if (event2.name == "gainFuchen") {
        return event2.fuchens?.length && player2.hasExpansions("olxianglv");
      }
      return event2.name != "phase" || game.phaseNumber == 0;
    },
    forced: true,
    async content(event2, trigger2, player2) {
      if (trigger2.name == "gainFuchen") {
        const cards3 = player2.getExpansions(event2.name).randomGets(trigger2.fuchens.length);
        if (cards3.length) {
  ...
  dcyingyou: {
    ...
    async cost(event2, trigger2, player2) {
      ...
      const result = await player2.chooseButton([`###${get.prompt("dcyingyou")}###选择一项并摸一张牌`, [list, "textbutton"]]).set("filterButton", ({ link }) => {
        ...
        const player3 = get.player();
        if (link == "card") {
          if (!player3.hasEquipableSlot(1) || player3.getEquip("real_zhuge")) {
            return 0;
          }
          const shaCount = player3.countCards("hs", (card2) => get.name(card2, player3) == "sha");
          return 3 + Math.min(2, shaCount);
        }
        if (link == "mantou") {
          const repeatable = player3.countCards("hs", (card2) => {
            const num = get.number(card2, player3);
            if (typeof num != "number" || num <= 0 || num > 10) {
              return false;
            }
            const type = get.type(card2, player3);
            return (type == "basic" || type == "trick") && !get.tag(card2, "norepeat");
          });
          return 1.5 + Math.min(3, repeatable) + (player3.countMark("dcyingyou") < 10 ? 1 : 0);
        }
        const skills2 = get.info("dcyingyou").getList().filter((skill) => !player3.hasSkill(skill[0], null, null, false));
        return skills2.length ? 2.5 : 0.5;
      }).forResult();
      if (result.bool) {
        event2.result = {
          bool: true,
    ...
    async content(event2, trigger2, player2) {
      switch (event2.cost_data) {
        case "skill": {
          const skills2 = get.info(event2.name).getList().filter((skill) => !player2.hasSkill(skill[0], null, null, false));
          if (skills2?.length) {
            const skill = skills2.randomGet();
            player2.flashAvatar(event2.name, skill[1]);
            await player2.addAdditionalSkills(event2.name, skill[0], true);
          }
          break;
        }
        case "card": {
        ...
        default: {
          player2.addMark(event2.name, 10, false);
    ...
    subSkill: {
      eat: {
        ...
        filter(event2, player2) {
          ...
          if (!get.info("dcshixian").filterx(event2)) {
            return false;
          }
        ...
        check(event2, player2) {
          ...
          const targets = event2.targets?.length ? event2.targets : [player2];
          const effect = targets.reduce((sum, target2) => sum + get.effect(target2, card2, player2, player2), 0);
          if (effect <= 0) {
            return false;
          }
          if (get.tag(card2, "recover") && targets.some((target2) => get.attitude(player2, target2) > 0 && target2.isDamaged())) {
            return true;
          }
        ...
        prompt2(event2, player2) {
          const num = get.number(event2.card);
          return `吃掉${num}吨馒头，令${get.translation(event2.card)}额外结算一次`;
        },
        async content(event2, trigger2, player2) {
  ...
  real_zhuge_skill: {
    ...
    filter(event2, player2) {
      ...
      return !event2.audioed && event2.card.name === "sha" && player2.countUsed("sha", true) > 1 && event2.getParent().type === "phase";
    },
    async content(event2, trigger2, player2) {
      const num = get.number(trigger2.card);
      if (typeof num == "number" && num < 7) {
  ...
  olandu: {
    ...
    filter(event2, player2, name, card2) {
    ...
    async cost(event2, trigger2, player2) {
      const cards2 = game.filterPlayer((current) => current != player2).reduce((cards3, current) => {
        return [...cards3, ...current.getCards("h", (card2) => get.info(event2.skill).isYinping(card2))];
      }, []);
      if (cards2?.length) {
    ...
    subSkill: {
      mark: {
        ...
        filter(event2, player2) {
          ...
          return event2.getg?.(player2)?.length;
        },
        charlotte: true,
        async cost(event2, trigger2, player2) {
          const cards2 = trigger2.name == "gameDraw" ? player2.getCards("h") : trigger2.getg(player2);
          get.info(event2.skill).initTag(player2, event2.skill, cards2);
        }
      }
    }
  },
  olqiqi: {
    ...
    round: 1,
    filter(event2, player2) {
      return event2.isFirstTarget && get.cardNameLength(event2.card) >= player2.hp;
    },
    async content(event2, trigger2, player2) {
  ...
  olhuaquan: {
    ...
    filter(event2, player2) {
      return event2.targets.some((target2) => target2 != player2) && event2.isFirstTarget;
    },
    forced: true,
    async content(event2, trigger2, player2) {
      const list = [
      ...
      const result = await player2.chooseButton([`###花拳###${get.skillInfoTranslation(event2.name, null, false)}`, [list, "vcard"]], true).set("ai", (button) => {
      ...
      if (choice == "olhuaquan_heavy") {
        ...
        player2.when("useCardAfter").filter((evt) => evt == trigger2.getParent()).step(async () => {
          await player2.draw();
        });
      }
      const targets = trigger2.targets.filter((target2) => target2 != player2);
      player2.line(targets);
      const chooseButton = async (target2) => {
        event2.target = target2;
        const result2 = await target2.chooseButton([`###花拳###猜测${get.translation(player2)}选择的效果`, [list, "vcard"]], true).set("ai", (button) => {
          const card2 = get.event().card;
          const bool = button.link == "olhuaquan_heavy";
          if (get.tag(card2, "damage") && get.type(card2) != "delay") {
            return bool ? 1 + Math.random() : 0.5 + Math.random();
        ...
        if (result2?.links?.[0]?.[2] != choice) {
          await event2.trigger("olhuaquan_wrong");
        }
      };
  ...
  olsanou: {
    ...
    filter(event2, player2) {
      if (event2.name == "damage") {
        return event2.source == player2 && event2.player != player2 && event2.player.isIn();
      }
      return event2.target.isIn();
    },
    ...
    async content(event2, trigger2, player2) {
      ...
      await player2.draw();
      target2.addMark(event2.name);
      if (target2.countMark(event2.name) >= 3 && !target2.hasSkill(event2.name + "_debuff")) {
        target2.clearMark(event2.name);
        target2.addSkill(event2.name + "_debuff");
        game.log(target2, "被击倒，简直毫无还手之力");
      }
    },
    subSkill: {
      debuff: {
        ...
        filter(event2, player2, name) {
          ...
          if (name.endsWith("End")) {
            return event2.olsanou_debuff?.length;
          }
          if (name.endsWith("Begin")) {
            return event2.cards.some((card2) => lib.skill.olsanou_debuff.filterCardx(card2, event2));
          }
          return event2.name == "lose" ? event2.position == ui.discardPile : true;
        },
        filterCardx(card2, event2) {
          if (event2.name == "gain") {
            if ((event2.getParent().name == "draw" || !get.owner(card2)) && card2.original == "c") {
              return true;
            }
          }
        ...
        async content(event2, trigger2, player2) {
          if (trigger2.name == "phaseUse") {
            ...
            let num = 0;
            if (name.endsWith("End")) {
              num += trigger2[event2.name]?.filter((card2) => trigger2.cards.includes(card2)).length;
            } else if (name.endsWith("Begin")) {
              trigger2.set(
                event2.name,
                trigger2.cards.filter((card2) => lib.skill[event2.name].filterCardx(card2, trigger2))
              );
              return;
            } else {
              num += trigger2.cards.length;
            }
      ...
      backup(result, player2) {
        return {
          ...
          async content(event2, trigger2, player3) {
            const num = get.info(event2.name)?.number;
            if (typeof num == "number") {
              player3.when("compare", false).filter((evt, player4) => {
                if (!evt.getParent(event2.name, true)) {
                  return false;
                }
                return [event2.player, event2.target].includes(player4);
              }).assign({
                firstDo: true
              }).step(async (event3, trigger3, player4) => {
                for (const [role, ind] of [
                  ["player", 1],
                  ["target", 2]
                ]) {
                  const current = trigger3[role];
                  if (current == player4) {
                    player4.logSkill("renhuoluan");
                    game.log(current, "拼点牌点数视为", `#y${get.strNumber(num, true)}`);
                    trigger3[`num${ind}`] = num;
                  }
                }
              }).finish();
            }
            const next = player3.chooseToCompare(event2.targets, (card3) => {
            ...
            if (minPlayer) {
              for (let target2 of players) {
                if (target2 == minPlayer || !minPlayer.isIn() || !target2.isIn()) {
                  continue;
                }
                const sha = new lib.element.VCard({ name: "sha", isCard: true });
                if (target2.canUse(sha, minPlayer, false)) {
                  await target2.useCard(sha, minPlayer, false);
                }
              }
            }
            if (maxPlayer) {
  ...
  renneyan: {
    ...
    async content(event2, trigger2, player2) {
      const bool = player2.getStorage(event2.name, false);
      ...
      if (bool) {
        ...
        const prompt = `弃置一张${get.translation(get.type2(trigger2.card, player2))}牌令${get.translation(trigger2.card)}额外结算一次，否则无效`;
        const result = await player2.chooseToDiscard(prompt, "he", (card2, player3) => {
          return get.type2(card2, player3) == get.event().cardType;
        }).set("cardType", get.type2(trigger2.card, player2)).set("ai", (card2) => {
          return 9 - get.value(card2);
        }).forResult();
        if (result?.bool) {
          if (get.info("dcshixian")?.filterx(trigger2)) {
            trigger2.effectCount++;
    ...
    async content(event2, trigger2, player2) {
      ...
      if (num >= 1) {
        player2.when({
          ...
          if (evt.card.name != "sha" || evt.getParent(event2.name) != event2) {
            return false;
          }
          ...
          const result = await player3.chooseTarget(
            "潜曜：为此【杀】额外指定一个目标",
            (cardx, player4, target2) => {
              const { targets, card: card3 } = get.event();
              if (targets.includes(target2)) {
                return false;
              }
              return lib.filter.targetEnabled2(card3, player4, target2) && lib.filter.targetInRange(card3, player4, target2);
            },
            true
          ).set("autodelay", true).set("ai", (target2) => {
            const event4 = get.event(), player4 = get.player(), trigger4 = event4.getTrigger();
            return get.effect(target2, trigger4.card, player4, player4);
          }).set("targets", trigger3.targets).set("card", trigger3.card).forResult();
          if (result.bool && result.targets?.length) {
            player3.line(result.targets, "green");
            game.log(result.targets, "成为了", trigger3.card, "的目标");
            trigger3.targets.addArray(result.targets);
          }
        });
      }
      if (num >= 2) {
        player2.when({
          ...
          if (typeof trigger3.baseDamage != "number") {
            trigger3.baseDamage = 1;
          }
          trigger3.baseDamage++;
          game.log(trigger3.card, "伤害+1");
        });
      }
      if (num >= 3) {
        next.set("oncard", () => {
    ...
    async content(event2, trigger2, player2) {
      if (trigger2.name == "draw") {
        ...
        if (player2.isMaxHp()) {
          ...
          if (card2) {
            trigger2.showCards.add(card2);
            await game.cardsGotoOrdering(card2).set("relatedEvent", trigger2);
          }
        }
        if (player2.isMinHp()) {
          let card2 = get.cardPile2((card3) => {
  ...
  stronglianwu: {
    ...
    filter(event2, player2) {
      ...
      if (!event2.target.countCards("he")) {
        return false;
      }
      return event2.player.getEquips(1).length || event2.getParent().jiu;
    },
    async cost(event2, trigger2, player2) {
      let num = 0;
      if (trigger2.player.getEquips(1).length) {
        num++;
      }
      if (trigger2.getParent().jiu) {
        num++;
      }
      event2.result = await trigger2.player.choosePlayerCard(get.prompt2(event2.skill, trigger2.target, trigger2.player), [1, num], "he", trigger2.target).set("ai", (button) => {
      ...
      event2.result.targets = [trigger2[player2 == trigger2.player ? "target" : "player"]];
    },
    async content(event2, trigger2, player2) {
      await trigger2.target.modedDiscard(event2.cards, trigger2.player);
    }
  },
  //张角三兄弟
  oltiangong: {
    ...
    filter(event2, player2) {
      if (event2.name == "judge") {
        return event2.result.suit == "spade";
      }
      return true;
    },
    async content(event2, trigger2, player2) {
      if (trigger2.name == "phase") {
        const name = event2.triggername == "phaseBegin" ? "leigong" : "younan", card2 = get.autoViewAs({ name, isCard: true });
        if (player2.hasUseTarget(card2, false, false)) {
          await player2.chooseUseTarget(card2, true, false);
        }
      } else {
        const result = await player2.chooseTarget(`天公：对一名不为${get.translation(trigger2.player)}的角色造成1点雷电伤害`, true, (card2, player3, target2) => {
          return get.event().sourcex != target2;
        }).set("sourcex", trigger2.player).set("ai", (target2) => get.damageEffect(target2, get.player(), get.player(), "thunder")).forResult();
        if (result?.targets) {
          const target2 = result.targets[0];
  ...
  oldigong: {
    ...
    forced: true,
    filter(event2, player2) {
      return game.hasPlayer2((target2) => {
    ...
    async content(event2, trigger2, player2) {
      ...
      if (get.is.damageCard(trigger2.card)) {
        ...
        player2.when("useCardAfter").filter((evt) => evt == trigger2).step(async (event3, trigger3, player3) => {
          ...
          const result = await target2.judge("oldigong", function(card2) {
  ...
  olrengong: {
    ...
    filter(event2, player2) {
      ...
      return get.type2(event2.card) != get.type2(last.card) && player2.countDiscardableCards(player2, "he");
    },
    async content(event2, trigger2, player2) {
      const last = player2.getLastUsed(1), type = ["basic", "trick", "equip"].removeArray([get.type2(trigger2.card), get.type2(last.card)])[0];
  ...
  dclieti: {
    ...
    async content(event2, trigger2, player2) {
      ...
      trigger2.gaintag[me.playerid] = (num, cards2) => {
  ...
  dcluankui: {
    ...
    filter(event2, player2) {
      if (event2.name == "damage") {
        return player2.getHistory("sourceDamage", (evt) => evt.num).indexOf(event2) == 1 && player2.countDiscardableCards(player2, "h", (card2) => card2.hasGaintag("yuanshaoyuanshu_shao"));
      } else {
        return event2.getg?.(player2)?.length && player2.getHistory("gain", (evt) => evt.cards.length).indexOf(event2) == 1 && player2.countDiscardableCards(player2, "h", (card2) => card2.hasGaintag("yuanshaoyuanshu_shu"));
      }
    },
    async cost(event2, trigger2, player2) {
      const name = trigger2.name, tag = name == "damage" ? "yuanshaoyuanshu_shao" : "yuanshaoyuanshu_shu";
      let str = `###${get.prompt(event2.skill)}###`;
      if (name == "damage") {
        str += "弃置一张「袁绍」牌令自己本回合下次造成的伤害翻倍";
      } else {
        str += "弃置一张「袁术」牌令自己本回合下次摸牌翻倍";
      }
      event2.result = await player2.chooseToDiscard(str, "h", "chooseonly", (card2) => card2.hasGaintag(get.event().tag)).set("tag", tag).set("ai", (card2) => 6 - get.value(card2)).forResult();
    },
    async content(event2, trigger2, player2) {
      const cards2 = event2.cards, name = trigger2.name;
  ...
  dcweiji: {
    ...
    trigger: { player: "useCardToPlayered" },
    filter(event2, player2) {
      return event2.isFirstTarget && event2.targets.some((i) => i !== player2);
    },
    async cost(event2, trigger2, player2) {
      event2.result = await player2.chooseTarget(get.prompt2(event2.skill), (card2, player3, target2) => {
        return target2 !== player3 && get.event().getTrigger().targets.includes(target2);
      }).set("ai", (target2) => {
        const player3 = get.player();
        return 2 + Math.sign(get.attitude(player3, target2)) + Math.random();
      }).forResult();
    },
    async content(event2, trigger2, player2) {
      const target2 = event2.targets[0], numbers = Array.from({ length: 3 }).map((_, i) => (i + 1).toString());
      const { control: num1 } = await player2.chooseControl(numbers).set("ai", () => {
        const { player: player3, target: target3 } = get.event().getParent();
        if (get.attitude(player3, target3) > 0 || get.attitude(target3, player3) > 0) {
          return 2;
        }
        return get.rand(0, 2);
      }).set("prompt", "请选择你给" + get.translation(target2) + "设下的难题").forResult();
      game.log(player2, "选择了一个数字");
      player2.chat("我选的" + [1, 2, 3, 114514, 1919810].randomGet() + "，你信吗");
      await game.delayx();
      const { control: num2 } = await target2.chooseControl(numbers).set("ai", () => {
        const { player: player3, target: target3 } = get.event().getParent();
        if (get.attitude(player3, target3) > 0 || get.attitude(target3, player3) > 0) {
          return 0;
        }
  ...
  dcsaima: {
    ...
    async cost(event2, trigger2, player2) {
      event2.result = await player2.chooseTarget(get.prompt2(event2.skill), (card2, player3, target2) => {
        ...
        const player3 = get.player();
        return get.damageEffect(target2, player3, player3) + Math.max(3, target2.countCards("h")) * get.effect(target2, { name: "guohe_copy", position: "h" }, player3, player3);
      }).forResult();
    },
    async content(event2, trigger2, player2) {
      const target2 = event2.targets[0];
  ...
  olyinfeng: {
    ...
    getIndex(event2, player2) {
      return game.filterPlayer((current) => {
        ...
        const cards2 = event2.getg?.(current);
        if (!cards2?.length) {
          return false;
        }
        return event2.getl?.(player2)?.hs?.some((card2) => cards2.includes(card2)) && (cards2.some((card2) => card2.name == "chixueqingfeng") || player2.countCards("h", { name: "chixueqingfeng" }));
      }).sortBySeat();
    },
    filter(event2, player2, name, target2) {
      if (event2.name === "loseAsync" && event2.type !== "gain") {
        return false;
      }
      return target2?.isIn();
    },
    ...
    async content(event2, trigger2, player2) {
      ...
      if (trigger2.getg(target2).some((card2) => card2.name == "chixueqingfeng")) {
        await player2.damage(target2);
      }
    },
      ...
      gain: {
        ...
        filter(event2, player2) {
          if (event2.name.indexOf("lose") == 0) {
            ...
            return event2.getd().some((card2) => card2.name == "chixueqingfeng" && get.position(card2, true) == "d");
          }
          return event2.name != "phase" || game.phaseNumber == 0;
        },
        async content(event2, trigger2, player2) {
          if (trigger2.name.indexOf("lose") == 0) {
            await player2.loseHp();
            const cards2 = trigger2.getd().filter((card2) => card2.name == "chixueqingfeng" && get.position(card2, true) == "d");
            if (cards2.length) {
              await player2.gain(cards2, "gain2");
            }
          } else {
  ...
  olfulu: {
    ...
    filter(event2, player2) {
      const { card: card2, player: target2, targets } = event2;
      if (card2.name != "sha" || !target2.countCards("h")) {
        return false;
      }
      if (player2 == target2) {
        return targets.some((i) => i.isIn());
      }
      return event2.olfulu_map?.[player2.playerid] && targets.includes(player2);
    },
    async cost(event2, trigger2, player2) {
      ...
      if (player2 == target2) {
        event2.result = await player2.chooseCardTarget({
          ...
          ai1(card2) {
            ...
            if (player3.countCards("h", { name: "chixueqingfeng" }) && player3.hasSkill("olyinfeng") && targets.some((target3) => get.damageEffect(target3, player3, player3) > 0)) {
              if (card2.name == "chixueqingfeng") {
                return 0;
              }
              return 6.5 - get.value(card2);
            }
            if (targets.some((target3) => get.effect(target3, { name: "shunshou_copy", position: "h" }, player3, player3) > 0)) {
              return 6.5 - get.value(card2);
            }
            return 0;
          },
          ai2(target3) {
            ...
            const { name } = cards2[0];
            const eff = get.effect(target3, { name: "shunshou_copy", position: "h" }, player3, player3) * Math.min(2, target3.countCards("h"));
            if (player3.countCards("h", { name: "chixueqingfeng" }) && name != "chixueqingfeng" && player3.hasSkill("olyinfeng")) {
              return get.damageEffect(target3, player3, player3) + eff;
            }
            return eff;
          }
        }).set(
          "targets",
          trigger2.targets.filter((i) => i.isIn())
        ).forResult();
      } else {
        event2.result = await target2.chooseCard("h", get.prompt(event2.skill), `交给${get.translation(player2)}一张手牌，然后获得其至多两张手牌`).set("ai", (card2) => {
          const { player: player3, target: target3 } = get.event();
          const att = get.attitude(player3, target3);
          if (att > 0) {
            const bool = target3.countCards("h", { name: "chixueqingfeng" });
            if (!target3.countCards("h")) {
              return 0;
            }
            return !bool && player3.needsToDiscard() ? 6 - get.value(card2) : 0;
          }
          return get.effect(target3, { name: "shunshou_copy", position: "h" }, player3, player3) > 0 ? 6 - get.value(card2) : 0;
    ...
    async content(event2, trigger2, player2) {
      ...
      if (player2 == target2) {
        const [target3] = targets;
        await player2.give(cards2, target3);
        if (target3.countGainableCards(player2, "h")) {
          await player2.gainPlayerCard(target3, "h", [1, 2], true);
        }
      } else {
        await target2.give(cards2, player2);
        if (player2.countGainableCards(target2, "h")) {
          await target2.gainPlayerCard(player2, "h", [1, 2], true);
        }
      }
    },
    group: "olfulu_record",
    subSkill: {
      record: {
        ...
        firstDo: true,
        filter(event2, player2) {
          const { card: card2, player: target2 } = event2;
          if (card2.name != "sha") {
            return false;
          }
          return target2 != player2 && target2.getHp() < player2.getHp();
        },
        content() {
  ...
  oljuejue: {
    ...
    forced: true,
    filter(event2, player2) {
      if (!["sha", "shan", "tao", "jiu"].includes(get.name(event2.card))) {
        return false;
      }
      return player2.getAllHistory("useCard", (evt) => get.name(evt.card) === get.name(event2.card)).indexOf(event2) == 0;
    },
    content() {
      trigger.baseDamage++;
      ...
      player.when({ player: "useCardAfter" }).filter((evt, player2, name2) => evt.card === trigger.card && evt.oljuejue).step(async (event2, trigger2, player2) => {
  ...
  olpimi: {
    ...
    filter(event2, player2) {
      if (!event2.card || event2.targets.length != 1 || !event2.player.isIn()) {
        return false;
      }
      return event2.player !== player2 && player2 === event2.targets[0] && event2.player.hasCard((card2) => lib.filter.canBeDiscarded(card2, player2, event2.player), "he") || event2.player === player2 && player2 !== event2.targets[0] && player2.hasCard((card2) => lib.filter.cardDiscardable(card2, player2, "olpimi"), "he");
    },
    async cost(event2, trigger2, player2) {
      let result, str = "的一张牌使此牌伤害或回复值+1，若使用者的手牌最多或最少，你摸一张牌且此技能本回合失效。";
      if (player2 === trigger2.player) {
        result = player2.chooseToDiscard("he", get.prompt(event2.skill), "弃置" + get.translation(trigger2.player) + str, "chooseonly").set("ai", (card2) => {
          const player3 = get.player();
        ...
        result = player2.discardPlayerCard("he", trigger2.player).set("chooseonly", true).set("prompt", get.prompt(event2.skill)).set("prompt2", "弃置" + get.translation(trigger2.player) + str).set("ai", (button) => {
          const player3 = get.player(), card2 = button.link;
          const event3 = get.event().getTrigger(), target2 = event3.player;
          ...
          if (get.tag(event3.card, "damage")) {
            eff += get.effect(player3, event3.card, target2, player3);
          }
          return eff > 0 ? get.value(card2) * (1 + Math.random()) : 0;
        });
      }
      event2.result = await result.forResult();
    },
    logTarget: "player",
    async content(event2, trigger2, player2) {
      await trigger2.player.modedDiscard(event2.cards, player2);
      trigger2.getParent().baseDamage++;
      if (trigger2.player.isMinHandcard() || trigger2.player.isMaxHandcard()) {
        await player2.draw();
  ...
  dcbaoshi: {
    ...
    async content(event2, trigger2, player2) {
      ...
      while (true) {
        ...
        const result = await player2.chooseControlList(
          get.translation(event2.name) + "：请选择一项（已亮出牌名字数之和为" + numx + "）",
          ["获得" + get.translation(cards2), "再亮出一张牌"],
          true
        ).set("ai", () => get.event().numx < 7 ? 1 : 0).set("numx", numx).forResult();
        if (result.index == 0) {
          await player2.gain(cards2, "gain2");
          break;
        }
  ...
  dcxinggong: {
    enable: "phaseUse",
    filter: (event2) => event2.dcshixinggong_cards?.length,
    onChooseToUse: (event2) => {
      if (game.online || event2.type !== "phase" || event2.dcshixinggong_cards) {
        return;
      }
      event2.set("dcshixinggong_cards", _status.discarded);
    },
    usable: 1,
    chooseButton: {
      dialog: (event2) => {
        return ui.create.dialog('###兴功###<div class="text center">请选择要从弃牌堆获得的牌</div>', event2.dcshixinggong_cards, "hidden");
      },
      select: [1, Infinity],
      allowChooseAll: true,
      check: (button) => {
  ...
  olsuichong: {
    ...
    filter(event2, player2) {
      ...
      return game.getAllGlobalHistory(
        "everything",
        (evt) => evt.name === "olsuichong" && evt.player === player2 && evt._trigger?.name === "phaseZhunbei"
      ).length < 3;
    },
    prompt2(event2, player2) {
    ...
    async content(event2, trigger2, player2) {
      ...
      const result = skills2.length > 1 ? await winner.chooseButton(
        [
          "岁崇：请选择一个生肖兽技能令" + get.translation(player2) + "获得",
          [
            skills2.map((skill2) => [
              skill2,
              '<div class="popup text" style="width:calc(100% - 10px);display:inline-block"><div class="skill">【' + get.translation(skill2) + "】</div><div>" + lib.translate[skill2 + "_info"] + "</div></div>"
            ]),
            "textbutton"
          ]
        ],
        true
      ).set("ai", () => 1 + Math.random()).forResult() : { links: skills2 };
  ...
  olshouhun: {
    ...
    filter(event2, player2) {
      ...
      if (event2.name === "phaseDraw") {
        return !event2.numFixed && storage[0] > 0;
      }
      return storage[2] > 0 && (event2.name !== "phase" || game.phaseNumber === 0);
    },
    forced: true,
    async content(event2, trigger2, player2) {
      const skill = event2.name, storage = player2.storage[skill];
      switch (trigger2.name) {
        case "damage": {
  ...
  olzishu: {
    ...
    selectTarget: 1,
    filter(event2, player2) {
      return game.hasPlayer((target2) => get.info("olzishu").filterTarget(null, player2, target2));
    },
    filterTarget(card2, player2, target2) {
      return target2 !== player2 && target2.countCards("h") > player2.countCards("h");
    },
    async content(event2, trigger2, player2) {
      await player2.gainPlayerCard(event2.target, "h", true);
      while (game.hasPlayer((target2) => get.info(event2.name).filterTarget(null, player2, target2)) && !player2.isMaxHandcard()) {
        const result = await player2.chooseTarget("是否继续获得手牌数大于你的一名角色的一张手牌？", get.info(event2.name).filterTarget).set("ai", function(target2) {
          const player3 = get.player();
          return get.effect(target2, { name: "shunshou_copy", position: "h" }, player3, player3);
        }).forResult();
        if (result.bool) {
    ...
    ai: {
      ...
      result: {
        player(player2, target2) {
          return get.effect(target2, { name: "shunshou_copy", position: "h" }, player2, player2);
        }
      }
    }
  },
      ...
      effect: {
        ...
        mod: {
          targetEnabled(card2, player2, target2) {
            if (player2 !== target2 && player2.getHp() >= target2.getHp()) {
              return false;
            }
          }
        },
  ...
  olchenlong: {
    ...
    async content(event2, trigger2, player2) {
      ...
      const result = await player2.chooseNumbers(get.translation(event2.name), [{ prompt: "请选择你要失去的体力值", min: 1, max: 2 }], true).set("processAI", () => {
        const player3 = get.player();
      ...
      if (num) {
        ...
        await event2.target.damage(num);
      }
    },
    ai: {
      order: 1,
    ...
    subSkill: {
      temp: {
        ...
        filter(event2, player2) {
          return event2.getParent("loseHp").name === "olchenlong";
        },
        content() {
          player.loseMaxHp();
        }
  ...
  olsishe: {
    ...
    check(event2, player2) {
      return get.damageEffect(event2.source, player2, player2) > 0;
    },
    logTarget: "source",
    content() {
    ...
    ai: {
      ...
      effect: {
        target(card2, player2, target2) {
          ...
          if (target2.hasFriend() && get.tag(card2, "damage")) {
            return [1, 0, 0, -0.7];
          }
        }
      }
  ...
  olwuma: {
    ...
    filter(event2, player2) {
      return event2.player !== player2 && get.type2(event2.card) === "trick";
    },
    forced: true,
    content() {
      player.draw();
  ...
  olweiyang: {
    ...
    filter(event2, player2) {
      ...
      return game.hasPlayer((target2) => target2.isDamaged());
    },
    filterCard(card2, player2) {
      if (!lib.filter.cardDiscardable(card2, player2)) {
        return false;
  ...
  olshenhou: {
    ...
    trigger: { target: "useCardToTargeted" },
    filter(event2, player2) {
      return event2.player !== player2 && event2.card.name === "sha";
    },
    check(event2, player2) {
  ...
  olyouji: {
    ...
    filter(event2, player2) {
      return game.roundNumber > 0 && !event2.numFixed;
    },
    forced: true,
    content() {
  ...
  olhaizhu: {
    ...
    filter(event2, player2) {
      ...
      return game.hasPlayer((target2) => {
        ...
        return event2.getl?.(target2)?.cards2?.some((card2) => get.color(card2) === "black" && get.position(card2, true) === "d");
      });
    },
    forced: true,
    async content(event2, trigger2, player2) {
      ...
      await player2.gain(
        game.filterPlayer((target2) => {
          ...
          return trigger2.getl?.(target2)?.cards2?.some((card2) => get.color(card2) === "black" && get.position(card2, true) === "d");
        }).reduce((list, target2) => {
          return list.addArray(
            trigger2.getl(target2).cards2.filter((card2) => get.color(card2) === "black" && get.position(card2, true) === "d")
          );
        }, []),
        "gain2"
      );
  ...
  olfengzhu: {
    ...
    async cost(event2, trigger2, player2) {
      event2.result = await player2.chooseTarget(
        (card2, player3, target2) => {
          return player3 != target2 && target2.hasSex("male") && !player3.getStorage("olfengzhu").includes(target2);
        },
        "逢主：义父你在哪儿？",
        ...
        const player3 = get.player();
        const att = get.attitude(player3, target2), num = target2.getHp();
        return (-att + 0.1) * get.effect(player3, { name: "draw" }, player3, player3) * num;
      }).forResult();
    },
    async content(event2, trigger2, player2) {
      const target2 = event2.targets[0], num = target2.getHp();
      player2.markAuto(event2.name, [target2]);
      await player2.draw(num);
      const [surname] = get.characterSurname(target2.name)[0];
      if (surname) {
        game.broadcastAll(
          (player3, surname2) => {
  ...
  olyuyu: {
    ...
    async cost(event2, trigger2, player2) {
      event2.result = await player2.chooseTarget(
        ...
        (card2, player3, target2) => {
          return player3.getStorage("olfengzhu").includes(target2);
        },
        "选择一名义父，令其饮恨",
        true
      ).set("ai", (target2) => {
    ...
    async content(event2, trigger2, player2) {
      ...
      target2.addMark(name);
      player2.addSkill(name + "_effect");
      player2.markAuto(name + "_effect", [target2]);
    },
    marktext: "恨",
    intro: {
      name: "恨(玉玉)",
    ...
    subSkill: {
      effect: {
        ...
        filter(event2, player2) {
          const target2 = _status.currentPhase;
          if (!target2 || !player2.getStorage("olfengzhu").includes(target2) || !target2.isIn()) {
            return false;
          }
          if (event2.name == "damage") {
            return event2.num > 0;
        ...
        async content(event2, trigger2, player2) {
          const num = trigger2.name == "damage" ? trigger2.num : trigger2.getl(player2).cards2.length;
  ...
  ollbzhiji: {
    ...
    filter(event2, player2) {
      return player2.getStorage("olfengzhu").includes(event2.target) && event2.target.hasMark("olyuyu");
    },
    forced: true,
    logTarget: "target",
    async content(event2, trigger2, player2) {
      const { card: card2, target: target2 } = trigger2;
      let num = target2.countMark("olyuyu");
  ...
  oljiejiu: {
    ...
    subSkill: {
      ...
      jiese: {
        ...
        filter(event2, player2) {
          if (!get.info("oljiejiu_jiese").logTarget(event2, player2).length) {
            return false;
          }
          return event2.name != "phase" || game.phaseNumber == 0;
        },
        forced: true,
        logTarget: (event2, player2) => game.filterPlayer((current) => {
        ...
        async content(event2, trigger2, player2) {
          for (const target2 of event2.targets.sortBySeat()) {
            const skills2 = target2.getStockSkills(true, true);
            if (!skills2.length) {
              continue;
            }
            const skill = skills2.randomRemove();
            await target2.changeSkills(["lijian"], [skill]);
            game.broadcastAll(
              (target3, skill2) => {
                for (const name of get.nameList(target3)) {
  ...
  dcbeijin: {
    ...
    subSkill: {
      ...
      buff: {
        ...
        content() {
          player.removeSkill(event.name);
          if (player.hasCard((card2) => card2.hasGaintag("dcbeijin_effect"))) {
            player.loseHp();
          }
        },
        mark: true,
  ...
  xingzhou: {
    ...
    filter(event2, player2) {
      ...
      if (!player2.canUse({ name: "sha" }, event2.source, false)) {
        return false;
      }
      return player2.countCards("h") > 1 && event2.player.isMinHandcard();
    },
    async cost(event2, trigger2, player2) {
      event2.result = await player2.chooseToDiscard("h", 2, get.prompt2(event2.skill, trigger2.source)).set("chooseonly", true).set("ai", (card2) => {
        const player3 = get.player(), target2 = get.event().getTrigger().source;
        if (!player3.canUse({ name: "sha" }, target2, false)) {
          return 0;
        }
        if (get.effect(target2, { name: "sha" }, player3, player3) <= 0) {
          return 0;
        }
        return 6 - get.value(card2);
      }).forResult();
      event2.result.targets = [trigger2.source];
    },
    async content(event2, trigger2, player2) {
      const { cards: cards2, targets } = event2;
      await player2.discard(cards2);
  ...
  lieshen: {
    ...
    onChooseToUse(event2) {
      ...
      if (_status.lieshen_map) {
        ...
        for (let key in map) {
          let target2 = game.findPlayer((current) => current.playerid == key);
          if (target2) {
            list.add([target2, ...map[key]]);
          }
        }
      }
      event2.set("lieshen_list", list);
    },
    filter(event2, player2) {
      const list = event2.lieshen_list;
      if (!list || !list.length) {
        return false;
      }
    ...
    async content(event2, trigger2, player2) {
      ...
      player2.awakenSkill(event2.name);
      const map = _status.lieshen_map[target2.playerid];
      if (map) {
        if (target2.hp > map[0]) {
          await target2.loseHp(target2.hp - map[0]);
        } else if (target2.hp < map[0]) {
          await target2.recoverTo(map[0]);
        }
        const num = target2.countCards("h");
        if (num > map[1]) {
          await target2.chooseToDiscard("h", num - map[1], true);
        } else if (num < map[1]) {
          await target2.drawTo(map[1]);
        }
      }
    },
    ai: {
      ...
      result: {
        target(player2, target2) {
          ...
          const map = list.find((key) => key[0] == target2);
          if (!map) {
            return 0;
          }
          let eff = 0, num1 = target2.hp - map[1], num2 = target2.countCards("h") - map[2];
          if (num1 > 0) {
            eff += get.effect(target2, { name: "losehp" }, target2, target2) * num1;
          } else if (num1 < 0) {
            eff -= get.recoverEffect(target2, target2, target2) * num1;
          }
          if (num2 > 0) {
            eff += get.effect(target2, { name: "guohe_copy2" }, target2, target2) * num2;
          } else if (num2 < 0) {
            eff -= get.effect(target2, { name: "draw" }, target2, target2) * num2;
          }
          if (Math.abs(eff) <= 5) {
            return 0;
          }
    ...
    subSkill: {
      init: {
        ...
        async cost(event2, trigger2, player2) {
          ...
          if (trigger2.name != "phase" && trigger2.player != player2) {
            targets = [trigger2.player];
          }
          let bool = targets.some((target2) => {
            if (!_status.lieshen_map) {
              return true;
            }
            return !_status.lieshen_map[target2.playerid];
          });
          event2.result = {
            bool,
            targets,
        ...
        async content(event2, trigger2, player2) {
          ...
          for (let target2 of event2.targets) {
            if (_status.lieshen_map[target2.playerid]) {
              continue;
            }
            _status.lieshen_map[target2.playerid] = [target2.hp, target2.countCards("h")];
          }
        }
      }
    }
  ...
  zhuzhou: {
    ...
    filter(event2, player2) {
      if (!event2.source || event2.source == event2.player) {
        return false;
      }
      if (!event2.source.isIn() || !event2.player.isIn()) {
        return false;
      }
      return event2.source.isMaxHandcard() && event2.player.countCards("h");
    },
    check(event2, player2) {
      return get.effect(event2.player, { name: "shunshou_copy2" }, event2.source, player2) > 0;
    },
    logTarget: "source",
    prompt2: "令其获得受伤角色的一张手牌",
    async content(event2, trigger2, player2) {
      await trigger2.source.gainPlayerCard(trigger2.player, "h", true);
    }
  },
  yaoxian: {
    enable: "phaseUse",
    ...
    delay: false,
    async content(event2, trigger2, player2) {
      const drawer = event2.targets[0], target2 = event2.targets[1];
      await drawer.draw(2);
      const result = await drawer.chooseToUse(
        ...
        "邀仙：对" + get.translation(target2) + "使用一张杀，否则失去1点体力"
          ...
          ai: {
            result: {
              target(player3, target2) {
                ...
                return ui.selected.cards.reduce((sum, card2) => sum += get.value(card2, target2), 0);
              }
            }
          }
        };
  ...
  fuzhao: {
    ...
    async content(event2, trigger2, player2) {
      const target2 = event2.targets[0];
      const result = await target2.judge(function(card2) {
  ...
  oldingxi: {
    ...
    filter(event2, player2) {
      if (!player2.getPrevious() || !event2.cards.filterInD("d").some((card2) => {
    ...
    async cost(event2, trigger2, player2) {
      const target2 = player2.getPrevious();
      const cards2 = trigger2.cards.filterInD("d").filter((card2) => get.is.damageCard(card2));
      event2.result = await player2.chooseButton([get.prompt2(event2.skill, target2), cards2]).set("filterButton", (button) => {
        const player3 = get.player(), target3 = get.event().target;
        return player3.canUse(button.link, target3);
      }).set("target", target2).set("ai", (button) => {
        const player3 = get.player(), target3 = get.event().target;
        return get.effect(target3, button.link, player3, player3);
      }).forResult();
    ...
    async content(event2, trigger2, player2) {
      player2.$gain2(event2.cards, false);
      await game.delayx();
      const useCardEvent = player2.useCard(event2.cards[0], event2.targets[0], false);
      await useCardEvent;
  ...
  olnengchen: {
    ...
    trigger: { player: "damageEnd" },
    filter(event2, player2) {
      return event2.card && player2.getExpansions("oldingxi").some((card2) => card2.name === event2.card.name);
    },
    forced: true,
    content() {
      const cards2 = player.getExpansions("oldingxi").filter((card2) => card2.name === trigger.card.name);
      player.gain(cards2.randomGet(), player, "give");
    },
    ai: { combo: "oldingxi" }
  },
  olhuojie: {
  ...
  dcjuanlv: {
    ...
    filter(event2, player2) {
      return player2.differentSexFrom(event2.target);
    }
  },
  dcqixin: {
    enable: "phaseUse",
    ...
    ai: {
      ...
      result: {
        player(player2) {
          ...
          const target2 = game.filterPlayer((i) => i != player2).sort((a, b) => {
          ...
          return player2.differentSexFrom(target2) ? 0 : 1;
        }
      }
    },
  ...
  olhuyi: {
    ...
    getBasic(event2, player2) {
      const name = event2.card.name;
    ...
    filter(event2, player2) {
      if (["useCard", "respond"].includes(event2.name)) {
        ...
        if (!get.info("olhuyi").getBasic(event2, player2).some((skill) => !player2.hasSkill(skill, null, null, false))) {
          return false;
        }
      ...
      return (event2.name != "phase" || game.phaseNumber == 0) && skills2.some((skill) => !player2.hasSkill(skill, null, null, false));
    },
    locked: false,
    async cost(event2, trigger2, player2) {
      if (["useCard", "respond"].includes(trigger2.name)) {
        ...
        const skills2 = get.info(event2.skill).getList().filter((skill) => !player2.hasSkill(skill, null, null, false)).randomGets(3);
        const list = [];
        ...
        event2.result = { bool: true, cost_data: links };
      }
    },
    async content(event2, trigger2, player2) {
      const skill = ["useCard", "respond"].includes(trigger2.name) ? get.info("olhuyi").getBasic(trigger2, player2).filter((skill2) => !player2.hasSkill(skill2, null, null, false)).randomGets(1) : event2.cost_data;
    ...
    subSkill: {
      remove: {
        ...
        async content(event2, trigger2, player2) {
          player2.changeSkills([], event2.cost_data).set("$handle", (player3, addSkills, removeSkills) => {
            game.log(
              player3,
              "失去了技能",
              ...removeSkills.map((i) => {
  ...
  dchuiwan: {
    ...
    async cost(event2, trigger2, player2) {
      let result = await player2.chooseButton([get.prompt2(event2.skill), [get.info(event2.skill).gainCards(player2), "vcard"]], [1, trigger2.num]).set("ai", (button) => {
  ...
  dchuanli: {
    ...
    filter(event2, player2) {
      ...
      }).length >= 3 || game.hasPlayer((target2) => {
        if (target2 == player2) {
          return false;
        }
        return player2.getHistory("useCard", (evt) => {
    ...
    async content(event2, trigger2, player2) {
      ...
      if (player2.getHistory("useCard", (evt) => {
        ...
        const result = await player2.chooseTarget(
          ...
          (card2, player3, target2) => {
            return target2 != player3 && !target2.hasSkill("dchuanli_zhangzhang");
          }
        ).set("ai", (target2) => {
          get.event().player;
          return get.rank("zhangzhang", true) - ["name", "name1", "name2"].reduce((sum, name) => {
            if (!target2[name] || !lib.character[target2[name]] || name == "name1" && target2.name1 == target2.name) {
              return sum;
            }
            return sum + get.rank(target2[name], true);
          }, 0);
        }).forResult();
        if (result.bool) {
          zhangzhang = true;
          const target2 = result.targets[0];
          await player2.logSkill("dchuanli", target2);
          target2.addTempSkill("dchuanli_zhangzhang", { player: "phaseAfter" });
          target2.markSkillCharacter("dchuanli_zhangzhang", "zhangzhang", "唤理-内事", "内事不决问张昭");
          await target2.addAdditionalSkills("dchuanli_zhangzhang", ["zhijian", "guzheng"]);
        }
      }
      const targets = game.filterPlayer((target2) => {
        if (target2 == player2 || target2.hasSkill("dchuanli_zhouyu")) {
          return false;
        }
        return player2.getHistory("useCard", (evt) => {
          return (evt.targets || []).includes(target2);
      ...
      if (targets.length) {
        ...
        if (result.bool) {
          ...
          const target2 = result.targets[0];
          await player2.logSkill("dchuanli", target2);
          target2.addTempSkill("dchuanli_zhouyu", { player: "phaseAfter" });
          target2.markSkillCharacter("dchuanli_zhouyu", "re_zhouyu", "唤理-外事", "外事不决问周瑜");
          await target2.addAdditionalSkills("dchuanli_zhouyu", ["reyingzi", "refanjian"]);
        }
      }
  ...
  dclisao: {
    ...
    multiline: true,
    async content(event2, trigger2, player2) {
      let targets = event2.targets.sortBySeat();
    ...
    ai: {
      ...
      result: {
        target(player2, target2) {
          ...
          if (target2.getStorage("dclisao_gaifa").includes(player2)) {
            return 0;
          }
          if (get.damageEffect(target2, player2, player2) < 0 && get.attitude(player2, target2) > 0) {
            return 0;
          }
          let cards2 = player2.getCards("hs", (card2) => get.tag(card2, "damage") && get.effect(target2, card2, player2, player2) > 0);
          if (!cards2.length) {
            return 0;
          }
          ...
          return cards2.reduce((sum, card2) => {
            if (player2.canUse(card2, target2)) {
              return sum + get.effect(target2, card2, player2, target2);
            }
            if (player2.canUse(card2, target2, false)) {
              return sum + get.effect(target2, card2, player2, target2) / 10;
            }
            return 0;
          }, 0) - 10;
        }
    ...
    subSkill: {
      gaifa: {
        ...
        filter(event2, player2) {
          ...
          return event2.name != "useCard" || targets.includes(event2.player);
        },
        forced: true,
        popup: false,
        async content(event2, trigger2, player2) {
          ...
          if (trigger2.name == "useCard") {
            ...
            trigger2.num = trigger2.num * (targets.length + 1);
          }
        },
        mark: true,
        marktext: "江",
      ...
      mark(dialog, storage, player2) {
        if (storage) {
          ...
          if (skill) {
            ...
            dialog.add(
              '<div><div class="skill">【' + get.translation(lib.translate[skill + "_ab"] || get.translation(skill).slice(0, 2)) + "】</div><div>" + get.skillInfoTranslation(skill, player2, false) + "</div></div>"
            );
          }
        } else {
  ...
  dczhanjiang: {
    ...
    filter(event2, player2) {
      return game.hasPlayer((target2) => {
        return target2.countCards("ej", (card2) => get.name(card2, false) == "qinggang" || get.name(card2, get.owner(card2)) == "qinggang");
      });
    },
    content() {
      let cards2 = [], targets = game.filterPlayer((target2) => {
        return target2.countCards("ej", (card2) => get.name(card2, false) == "qinggang" || get.name(card2, get.owner(card2)) == "qinggang");
      });
      targets.forEach(
        (target2) => cards2.addArray(target2.getCards("ej", (card2) => get.name(card2, false) == "qinggang" || get.name(card2, get.owner(card2)) == "qinggang"))
      );
  ...
  dcshuangbi: {
    ...
    async content(event2, trigger2, player2) {
      ...
      var result = await player2.chooseControl().set("choiceList", [
        ...
        if (!game.hasPlayer((target3) => player3.canUse(card3, target3) && get.effect(target3, card3, player3, player3) > 0)) {
          return 0;
        }
        return 2;
      }).forResult();
      ...
      switch (result.index) {
        ...
        case 1:
          ...
          if (result2.bool) {
            ...
            var targets = game.filterPlayer((target3) => target3 != player2);
            if (targets.length) {
              while (sum) {
                sum--;
                var target2 = targets.randomGet();
                player2.line(target2);
                target2.damage(1, "fire");
                game.delayx();
              }
            }
          }
          break;
        case 2:
          while (num && game.hasPlayer((target3) => player2.canUse({ name: "sha", nature: "fire" }, target3) || player2.canUse({ name: "huogong" }, target3))) {
            num--;
            var list = [];
            if (game.hasPlayer((target3) => player2.canUse({ name: "sha", nature: "fire" }, target3))) {
              list.push(["基本", "", "sha", "fire"]);
            }
            if (game.hasPlayer((target3) => player2.canUse({ name: "huogong" }, target3))) {
    ...
    async content(event2, trigger2, player2) {
      ...
      if (hp >= 3) {
        if (player2.hasHistory("useSkill", (evt) => {
          var evtx = evt.event;
          return evt.skill == "dcsantou" && evtx.getTrigger().source == source && evtx.getParent(2) != trigger2;
        })) {
          lose = true;
        }
      } else if (hp == 2) {
        if (trigger2.hasNature()) {
          lose = true;
        }
      } else if (hp == 1) {
        if (trigger2.card && get.color(trigger2.card) == "red") {
          lose = true;
        }
      }
      if (lose) {
    ...
    ai: {
      ...
      effect: {
        target(card2, player2, target2) {
          ...
          if (get.tag(card2, "damage")) {
            const hp = target2.getHp();
            player2._dcsantou_temp = true;
            const losehp = get.effect(target2, { name: "losehp" }, target2, target2) / get.attitude(target2, target2);
            delete player2._dcsantou_temp;
            if (hp >= 3) {
              if (target2.hasHistory("useSkill", (evt) => evt.skill == "dcsantou" && evt.event.getTrigger().source == player2)) {
                return [0, losehp, 0, 0];
              } else if (get.attitude(player2, target2) < 0) {
  ...
  dcjianxiong: {
    ...
    async content(event2, trigger2, player2) {
      if (get.itemtype(trigger2.cards) == "cards" && get.position(trigger2.cards[0], true) == "o") {
        await player2.gain(trigger2.cards, "gain2");
      }
      ...
      effect: {
        target(card2, player2, target2) {
          if (get.tag(card2, "damage") && player2 != target2) {
            ...
            if (get.value(cards2, target2) >= 7 - player2.countMark("dcjianxiong") / 2 + target2.getDamagedHp()) {
  ...
  dcrende: {
    ...
    delay: false,
    filterTarget(card2, player2, target2) {
      if (player2.getStorage("dcrende_targeted").includes(target2)) {
        return false;
      }
      return player2 != target2 && target2.countGainableCards(player2, "h") > 1;
    },
    async content(event2, trigger2, player2) {
      player2.addTempSkill("dcrende_targeted", "phaseUseAfter");
      player2.markAuto("dcrende_targeted", [event2.target]);
      await player2.gainPlayerCard(event2.target, "h", true, 2);
      var list = [];
      for (var name of lib.inpile) {
        ...
        if (lib.filter.cardUsable(card2, player2, event2.getParent("chooseToUse")) && game.hasPlayer((current) => {
  ...
  dczhiheng: {
    ...
    async content(event2, trigger2, player2) {
      ...
      await player2.draw(num + event2.cards.length);
    },
    subSkill: {
      add: {
        audio: "dczhiheng",
        ...
        filter(event2, player2) {
          ...
          return !player2.getStorage("dczhiheng_hit").includes(event2.player);
        },
        logTarget: "player",
        content() {
  ...
  dcbianzhuang: {
    ...
    ai: {
      ...
      effect: {
        target_use(card2, player2, target2, current) {
          if (player2 == target2 && player2.isPhaseUsing() && get.type(card2) == "equip") {
    ...
    subSkill: {
      refresh: {
        ...
        forced: true,
        filter(event2, player2) {
          return get.type2(event2.card, false) == "equip" && typeof player2.getStat("skill").dcbianzhuang == "number";
        },
        content() {
  ...
  dctongliao: {
    ...
    subSkill: {
      draw: {
        ...
        filter(event2, player2) {
          ...
          if (event2.name == "lose") {
            for (var i in event2.gaintag_map) {
              if (event2.gaintag_map[i].includes("dctongliao")) {
                return true;
              }
            }
            return false;
        ...
        content() {
          var num = 0;
          var cards2 = trigger.getl(player).hs, ids = [];
          if (trigger.name == "lose") {
            ...
            player.getHistory("lose", function(evt) {
              ...
              for (var i2 in evt.gaintag_map) {
                if (evt.gaintag_map[i2].includes("dctongliao")) {
                  ids.push(i2);
                }
              }
            });
  ...
  dcwudao: {
    ...
    frequent: true,
    filter(event2, player2) {
      if (player2.getStorage("dcwudao_effect").includes(get.type2(event2.card, false))) {
        return false;
      }
      var history = player2.getHistory("useCard"), index = history.indexOf(event2);
      if (index < 1) {
        return false;
      }
      var evt = history[index - 1];
      return get.type2(event2.card, false) == get.type2(evt.card, false);
    },
    prompt2(event2) {
      return "令你本回合使用" + get.translation(get.type2(event2.card, false)) + "牌时不可被响应且伤害+1";
    },
    content() {
    ...
    subSkill: {
      effect: {
        ...
        onremove: true,
        filter(event2, player2) {
          return player2.getStorage("dcwudao_effect").includes(get.type2(event2.card, false));
        },
        content() {
  ...
  clbjisu: {
    ...
    async content(event2, trigger2, player2) {
      ...
      const result = await player2.chooseTarget(get.prompt("clbjisu"), "跳过判定阶段和摸牌阶段，视为对一名其他角色使用一张【杀】", function(card2, player3, target2) {
        if (player3 == target2) {
          return false;
        }
  ...
  dcshixian: {
    ...
    filter(event2, player2) {
      ...
      var evt = history[index - 1];
      return get.is.yayun(get.translation(event2.card.name), get.translation(evt.card.name));
    },
    filterx(event2) {
    ...
    prompt2(event2, player2) {
      if (lib.skill.dcshixian.filterx(event2)) {
        return "摸一张牌并令" + get.translation(event2.card) + "额外结算一次？";
      }
      return "摸一张牌。";
    },
    check(event2, player2) {
  ...
  dclonggong: {
    ...
    logTarget: "source",
    check(event2, player2) {
      return get.attitude(player2, event2.source) >= 0 || player2.hp <= Math.max(2, event2.num);
    },
    async content(event2, trigger2, player2) {
      trigger2.cancel();
  ...
  dcsitian: {
    ...
    async content(event2, trigger2, player2) {
      ...
      const result = await player2.chooseButton(true, [
        "请选择执行一个天气",
        [
          list.map((i) => [
            i,
    ...
    weathers: {
      ...
      雷电: {
        ...
        async content(event2, trigger2, player2) {
          ...
          for (const target2 of targets) {
            ...
            const result = await target2.judge(lib.card.shandian.judge, get.translation("shandian")).set("judge2", lib.card.shandian.judge2).forResult();
            var name = "shandian";
            if (event2.cancelled && !event2.direct) {
      ...
      大浪: {
        ...
        async content(event2, trigger2, player2) {
          ...
          for (const target2 of targets) {
            if (target2.isIn()) {
              var num = target2.countCards("e");
              if (num > 0) {
                await player2.discardPlayerCard(target2, true, "e", num);
              } else {
      ...
      暴雨: {
        ...
        async content(event2, trigger2, player2) {
          ...
          if (result.bool) {
            ...
            if (num > 0) {
              player2.discardPlayerCard(target2, true, "h", num);
            } else {
              target2.loseHp();
              await game.delayex();
            }
      ...
      大雾: {
        ...
        content() {
          ...
          player.line(targets);
          for (var target2 of targets) {
            target2.addSkill("dcsitian_dawu");
          }
        },
        ai(player2) {
          var effect = 0;
  ...
  dccibei: {
    ...
    logTarget: "player",
    filter(event2, player2) {
      return player2 != event2.player && !player2.hasHistory("useSkill", function(evt) {
        return evt.skill == "dccibei" && evt.targets.includes(event2.player);
      });
    },
    check(event2, player2) {
  ...
  dcruyi: {
    ...
    forced: true,
    filter(event2, player2) {
      return (event2.name != "phase" || game.phaseNumber == 0) && player2.hasEquipableSlot(1) && !player2.getEquips("ruyijingubang").length;
    },
    content() {
      var card2 = game.createCard2("ruyijingubang", "heart", 9);
    ...
    subSkill: {
      blocker: {
        ...
        forced: true,
        filter(event2, player2) {
          if (event2.name == "disableEquip") {
            return event2.slots.includes("equip1");
          }
          var cards2 = player2.getEquips("ruyijingubang");
          return event2.cards.some((card2) => cards2.includes(card2));
        },
        content() {
          if (trigger.name == "lose") {
    ...
    ai: {
      ...
      effect: {
        player(card2, player2, target2, current) {
          if (get.tag(card2, "damage") > 0 && player2 != target2) {
            ...
            if (player2.hasSkill("dccibei") && !player2.hasHistory("useSkill", function(evt) {
              return evt.skill == "dccibei" && evt.targets.includes(target2);
            })) {
              return [1, 3];
            }
          }
  ...
  ruyijingubang_effect: {
    ...
    filter(event2, player2) {
      ...
      if (game.hasPlayer(function(current) {
        return !event2.targets.includes(current) && lib.filter.targetEnabled2(card2, player2, current) && lib.filter.targetInRange(card2, player2, current);
      })) {
        return true;
      }
      return false;
    ...
    async content(event2, trigger2, player2) {
      ...
      if (num == 4) {
        const result = await player2.chooseTarget(
          ...
          "为" + get.translation(trigger2.card) + "额外指定一个目标",
          function(card2, player3, target2) {
            return !_status.event.sourcex.includes(target2) && player3.canUse(_status.event.card, target2, false);
          }
  ...
  dcnutao: {
    ...
    content() {
      var target2 = trigger.targets.filter((i) => i != player).randomGet();
      player.line(target2, "thunder");
      target2.damage("thunder");
    },
    ai: {
      effect: {
        player_use(card2, player2, target2) {
          if (player2 !== target2 && get.type2(card2) === "trick") {
            let tars = [target2];
            if (ui.selected.targets.length) {
              tars.addArray(ui.selected.targets.filter((i) => i !== player2 && i !== target2));
            }
            if (tars.length < 2) {
              return [1, 0, 1, -2];
            }
    ...
    subSkill: {
      add: {
        ...
        filter(event2, player2) {
          return event2.nature == "thunder" && player2.isPhaseUsing();
        },
        forced: true,
        content() {
  ...
  spduanzhi: {
    ...
    check(event2, player2) {
      var target2 = event2.player;
      if (get.attitude(player2, target2) >= -2 || target2.countCards("he", function(card2) {
    ...
    filter(event2, player2) {
      return player2 != event2.player && event2.player.countDiscardableCards(player2, "he") > 0;
    },
    content() {
  ...
  spduyi: {
    ...
    async content(event2, trigger2, player2) {
      ...
      const result = await player2.chooseTarget("令一名角色获得" + get.translation(card2), true).set("ai", function(target3) {
        var att = get.attitude(_status.event.player, target3);
        if (_status.event.du) {
          if (target3.hasSkillTag("nodu")) {
            return 0;
          }
          return -att;
        }
        if (att > 0) {
          ...
          return att + Math.sqrt(Math.max(0, 5 - target3.countCards("h")));
        }
        return att;
    ...
    subSkill: {
      gain: {
        ...
        filter(event2, player2) {
          ...
          return player2.isTurnedOver() && player2 != _status.currentPhase && event2.getg?.(player2)?.length > 0;
        },
        check(event2, player2) {
      ...
      lose: {
        ...
        check(event2, player2) {
          ...
          if (target2.countCards("e", (card2) => get.value(card2, target2) <= 0)) {
            return att > 0;
          }
          return att < 0;
        },
  ...
  spmixin: {
    ...
    async content(event2, trigger2, player2) {
      ...
      await player2.give(event2.cards, targets[0]);
      ...
      ).set("complexSelect", true).set("filterTarget", function(card2, player3, target2) {
    ...
    ai: {
      ...
      result: {
        target(player2, target2) {
          ...
          if (ui.selected.targets.length == 0) {
            if (card2.name == "sha" || target2.hasSha()) {
              return 2;
            }
            if (get.value(card2, target2) < 0) {
              return -2;
            }
            return 0;
          }
          var target1 = ui.selected.targets[0];
          if ((card2.name == "sha" || target1.hasSha()) && get.effect(target2, { name: "sha" }, target1, target1) > 0) {
            return get.effect(target2, { name: "sha" }, target1, target2);
          }
          return 1.5;
        }
      }
  ...
  spfengyin: {
    ...
    direct: true,
    filter(event2, player2) {
      return player2 != event2.player && event2.player.hp >= player2.hp && player2.countCards("h", function(card2) {
        if (_status.connectMode) {
          return true;
        }
    ...
    async content(event2, trigger2, player2) {
      const result = await player2.chooseCard(
        "h",
        get.prompt("spfengyin", trigger2.player),
        ...
        (function() {
          ...
          if (trigger2.player.countCards("hs") < trigger2.player.hp) {
            return false;
          }
          return true;
        })()
      ...
      if (result.bool) {
        ...
        player2.logSkill("spfengyin", target2);
        player2.give(result.cards, target2, "give");
        target2.skip("phaseUse");
        target2.skip("phaseDiscard");
      }
    }
  },
  spchizhong: {
  ...
  fenxin_old: {
    ...
    filter(event2, player2) {
      ...
      return event2.player.identity != "zhu" && player2.identity != "zhu" && player2.identity != "mingzhong" && event2.player.identity != "mingzhong";
    },
    check(event2, player2) {
      if (player2.identity == event2.player.identity) {
      ...
      switch (player2.identity) {
        ...
        case "zhong":
          ...
          return true;
        case "nei":
          if (event2.player.identity == "fan" && stat < 0) {
            return true;
          }
          if (event2.player.identity == "zhong" && stat > 0) {
            return true;
          }
          if (stat == 0) {
    ...
    prompt(event2, player2) {
      return "焚心：是否与" + get.translation(event2.player) + "交换身份？";
    },
    content() {
      game.broadcastAll(
        function(player2, target2, shown) {
          var identity = player2.identity;
          player2.identity = target2.identity;
  ...
  quanjia: {
    ...
    filter(event2, player2) {
      if (!event2.source || event2.source == player2 || !event2.player || event2.player == event2.source || _status.currentPhase != event2.source) {
        return false;
      }
      if (!event2.source.isIn()) {
        return false;
      }
      return event2.source.countCards("h") > 0;
    },
    logTarget: "source",
    prompt2(event2, player2) {
    ...
    async content(event2, trigger2, player2) {
      ...
      const cards2 = target2.getCards("h").randomGets(num);
      player2.clearMark(skill, false);
      if (cards2.length) {
        await player2.viewCards(get.translation(target2) + "的手牌", cards2);
        const shaCards = cards2.filter((card2) => {
          return get.name(card2, target2) == "sha";
        });
        if (shaCards.length) {
...
const dynamicTranslates = {
  oldianzan(player2) {
    const targets = player2.getStorage("oldianzan").filter((target2) => target2?.isIn());
    let str = "刘禅";
    if (targets?.length) {
      str = targets.map((target2) => get.rawName(target2.name)).join("、");
      if (targets.length > 1) {
        str += "中的一人";
      }
    }
