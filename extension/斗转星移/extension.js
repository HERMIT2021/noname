...
import { Character3 } from "./class/Mvp.js"
//自用
...
game.import("extension",function(){return {name:"斗转星移",content:function(config,pack){
  ...
  "backupBancharData": {
    ...
    async onclick() {
      ...
      for (let mode of lib.config.all.mode) {
        let planIndex = banChar.getPlanIndexByMode(mode);
        let banList = lib.config[`${dzxy.dz}plan${planIndex}`]?.banList || lib.config[mode + '_banned'] || [];
        let str = JSON.stringify(banList);
  ...
  "openDsplashPage": {
    name: '<button class="dz-menu-config-btn">打开启动页</button>',
    clear: true,
    onclick() {
      Dsplash.openPage();
    }
  ...
  "xiaotouxiang": {
    "name": "手杀小头像读取",
    "intro": `禁将界面显示手杀样式小头像(需要手动往扩展内image/char补充素材)`,
    "init": false,
  },
  ...
  "qianhuanSkinDir": {
    ...
    init: "sanguoskin",
    item: {
      "sanguoskin": "sanguoskin",
      "sanguolutouskin": "sanguolutouskin",
    },
  },
  "skinLoutou": {
    name: "皮肤露头样式",
