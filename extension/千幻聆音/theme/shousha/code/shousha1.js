...
window.qhly_import(function(lib, game, ui, get, ai, _status){
    game.qhly_initShoushaView = function (name, view, page, cplayer) {
        ...
        shoushaBg.addEventListener(lib.config.touchscreen ? 'touchend' : 'mouseup', function () {
          ...
          if (shoushaBg.length) {
            if (!shoushaBg[0].classList.includes('suoxiao')) return;
        ...
        for (var i = 0; i < 3; i++) {
          if (i == 0 || i == 1 && hasGuozhan || i == 2 && hasJjc) {
            ...
            skintype[i].listen(function () {
              if (this.classList.includes('sel')) return;
        ...
        subView.hpBackground = ui.create.div('.qh-hp-background', view);
       ...
       if (dibuhuo) {
        ...
        subView.avatarImage.addEventListener(lib.config.touchscreen ? "touchstart" : "mousedown", function (e) {
          ...
          if (!subView.avatarImage.dynamic || subView.avatarImage.dynamic && subView.avatarImage.dynamic.primary == null) {
            if (!subView.avatarImage.classList.includes('qh-image-standard')) return;
          }
        ...
        subView.dynamicToggle.addEventListener(lib.config.touchscreen ? 'touchend' : 'click', function () {
          ...
          if (this.classList.includes('jing')) {
            ...
            init: function (name, state) {
              ...
              conversion.addEventListener('click',function(){
                if(MovingRight){
                  ...
                  that.taici.show();
                  Bgbody.hide();
                  subView.hpBackground.style.display = 'none';
                  MovingRight = false;
                }else{
                  Allbuttons.forEach(bt=>{
                    if(bt.show)bt.show();
                  ...
                  that.taici.hide();
                  Bgbody.show();
                  subView.hpBackground.style.display = '';
                  MovingRight = true;
                }
              })
            ...
            if(abo){
              if(!document.getElementById('qhlySkinChangebutton1')){
                ...
                const radios = width1/height1;
                   if(radios <= 1){
                    if( state.mainView.dynamicToggle.classList.includes('jing')||subView.avatarImage.dynamic && subView.avatarImage.dynamic.primary == null){
