...
Array.prototype.remove = function (item) {//获取元素在数组中的索引
    var index = this.indexOf(item);
    if (index >= 0) return this.splice(index, 1);//使用splice删除元素，返回包含被删除元素的数组
    return item;//无匹配项时返回原始数据
}
...
onmessage = function (e) {
    ...
    switch (data.message) {
        case 'CREATE':
            ...
            var dynamic = new newDuilib.AnimationPlayer(data.pathPrefix, 'offscreen', data.canvas);
            dynamic.id = data.id;
