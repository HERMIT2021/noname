...
function skinSwitchSetUnpackUpload(image, on) {
    if (!image) return
    if (on) skinSwitchUnpackImageMap.set(image, true)
    else skinSwitchUnpackImageMap.delete(image)
    try {
        if (on) image.__skinSwitchUnpackPremultiply = true
        else delete image.__skinSwitchUnpackPremultiply
    } catch (e) { /* ImageBitmap 等 */ }
}
function skinSwitchGetUnpackUpload(image) {
    if (!image) return false
    try {
        if (image.__skinSwitchUnpackPremultiply) return true
    } catch (e) {}
    return !!skinSwitchUnpackImageMap.get(image)
}
...
const Ani4_2_StartId = 90000  // 4.2版本之前的spine内部维护的起始id
...
function spineApplyWorldTransform(skeleton, deltaSec) {
    if (!skeleton || !skeleton.updateWorldTransform) return;
    if (skeleton.updateWorldTransform.length > 0) {
        var Ph = self.spine_4_2 ? self.spine_4_2.Physics : null;
        const usePhysics = skeleton.physicsConstraints && skeleton.physicsConstraints.length > 0;
        if (usePhysics && deltaSec != null && typeof skeleton.update === 'function') skeleton.update(deltaSec);
        skeleton.updateWorldTransform(Ph ? (usePhysics ? Ph.update : Ph.none) : (usePhysics ? 2 : 0));
    } else {
        skeleton.updateWorldTransform();
    }
}
self.spineApplyWorldTransform = spineApplyWorldTransform;
...
class BaseAPNode {
    constructor(initParam) {
        ...
        this.replaceSlots = initParam.replaceSlots;
        ...
        this.unpackPremultipliedAlpha = !!initParam.unpackPremultipliedAlpha  // 上传纹理时 unpack 预乘
        this.mvp = undefined  // 控制变换平移
        this.viewportNode = initParam.viewportNode  // 用来单独控制viewport裁剪用的node节点
        this.holdLastFrame = initParam.holdLastFrame;
    }
    ...
    update (e) {
        ...
        if (domNode) {
            if (this.referFollow || !this.referBounds) {
                ...
                let y
                if (rect.bottomInBody != null && rect.canvasBottomInBody != null) {
                    y = rect.bottomInBody - rect.canvasBottomInBody
...
class BaseAnimation {
    ...
    removeSpine(filename, skelType) {
        ...
        if (this.hasSpine(filename)) {
            let pp = (this.spine.assetManager && this.spine.assetManager.pathPrefix) ? this.spine.assetManager.pathPrefix : ''
            for (let suffix of ['.png', '.atlas', '.' + skelType]) {
                this.spine.assetManager.remove(filename + suffix)
                if (typeof self !== 'undefined' && self.spine && self.spine.lodedAssets) {
                    delete self.spine.lodedAssets[pp + filename + suffix]
                }
            }
    ...
    recordSpineAssetTextureUnpack(filename) {
        let a = this.spine.assets[filename]
        if (a) a.textureUnpackPremultipliedAlpha = !!this.unpackPremultipliedAlpha
    }

    refreshLoadedSpineUnpackTextures(filename, wantUnpack) {
        wantUnpack = !!wantUnpack
        const manager = this.spine && this.spine.assetManager
        if (!manager || !filename) return false
        const atlas = manager.get(filename + '.atlas')
        if (!atlas || !atlas.pages || !atlas.pages.length) return false
        const gl = this.gl
        const useMipDefault = gl && gl.useMipMaps
        let any = false
        for (let i = 0; i < atlas.pages.length; i++) {
            const page = atlas.pages[i]
            const tex = page.texture
            if (!tex || typeof tex.update !== 'function') continue
            let img = tex._image
            if (img == null && typeof tex.getImage === 'function') img = tex.getImage()
            if (!img) continue
            const isBmp = typeof ImageBitmap !== 'undefined' && img instanceof ImageBitmap
            if (wantUnpack && isBmp && skinSwitchDecodedPremultiplyBmps.has(img)) {
                skinSwitchSetUnpackUpload(img, false)
            } else {
                skinSwitchSetUnpackUpload(img, wantUnpack)
            }
            const uMip = tex.useMipMaps != null ? tex.useMipMaps : useMipDefault
            try {
                tex.update(uMip)
                any = true
            } catch (e) {
                console.warn('refreshLoadedSpineUnpackTextures', filename, e)
                return false
            }
        }
        if (!any) return false
        const a = this.spine.assets[filename]
        if (a) a.textureUnpackPremultipliedAlpha = wantUnpack
        return true
    }

    static resolveReplaceSlotImage(tex) {
        if (tex == null) return null;
        if (typeof ImageBitmap !== 'undefined' && tex instanceof ImageBitmap) return tex;
        if (typeof HTMLImageElement !== 'undefined' && tex instanceof HTMLImageElement) return tex;
        if (typeof HTMLCanvasElement !== 'undefined' && tex instanceof HTMLCanvasElement) return tex;
        if (typeof OffscreenCanvas !== 'undefined' && tex instanceof OffscreenCanvas) return tex;
        try {
            if (typeof Laya !== 'undefined' && Laya.Texture && tex instanceof Laya.Texture) {
                let s = tex.source;
                if (s) return s;
                let b = tex.bitmap;
                if (b) return b.source || b._source || b._image;
            }
        } catch (e) {}
        if (tex.source != null) return BaseAnimation.resolveReplaceSlotImage(tex.source);
        if (tex.bitmap != null) {
            if (tex.bitmap.source) return tex.bitmap.source;
            if (tex.bitmap._source) return tex.bitmap._source;
        }
        if (tex.baseTexture != null && tex.baseTexture.resource) {
            let r = tex.baseTexture.resource;
            if (r.source) return r.source;
        }
        return tex;
    }

    static expandRendererSlotList(skeleton, list) {
        if (list == null) return list;
        if (!skeleton || typeof skeleton.findSlot !== 'function') return list;
        const raw = Array.isArray(list) ? list : [list];
        const out = [];
        function findSlotByName(sk, nm) {
            var s = sk.findSlot(nm);
            if (s) return s;
            if (sk.slots) {
                for (var j = 0; j < sk.slots.length; j++) {
                    var sl = sk.slots[j];
                    if (sl && sl.data && sl.data.name === nm) return sl;
                }
            }
            return null;
        }
        for (let i = 0; i < raw.length; i++) {
            ...
            const slot = findSlotByName(skeleton, key);
            if (slot) {
                let att = slot.getAttachment();
                if (!att && slot.data && slot.data.attachmentName != null && typeof skeleton.getAttachment === 'function') {
                    try {
                        att = skeleton.getAttachment(slot.data.index, slot.data.attachmentName);
                    } catch (e) {
                        att = null;
                    }
                }
                if (att && att.name != null && String(att.name).length > 0) {
                    if (out.indexOf(att.name) === -1) out.push(att.name);
                }
            } else if (out.indexOf(key) === -1) {
                out.push(key);
            }
        }
    ...
    replaceSlotTexture(nodeOrSkeleton, slotName, image, regionName) {
        if (!this.spineLib) {
            return false;
        }
        const skeleton = nodeOrSkeleton && nodeOrSkeleton.skeleton != null
            ? nodeOrSkeleton.skeleton
            : nodeOrSkeleton;
        ...
        const SL = this.spineLib;
        const texName = regionName != null ? regionName : ('replace_' + slotName);
        let atlasRegion;
        try {
            atlasRegion = this.createTextureRegion(image, texName, nodeOrSkeleton);
        } catch (e) {
            return false;
        }

        if (!att) {
            if (!SL.RegionAttachment) {
                return false;
            }
            let nm = slotName;
            if (slot.data && slot.data.attachmentName != null && String(slot.data.attachmentName).length > 0) {
                nm = slot.data.attachmentName;
            }
            const raw = new SL.RegionAttachment(nm);
            const ow = atlasRegion.originalWidth != null ? atlasRegion.originalWidth : atlasRegion.width;
            const oh = atlasRegion.originalHeight != null ? atlasRegion.originalHeight : atlasRegion.height;
            raw.width = ow;
            raw.height = oh;
            raw.setRegion(atlasRegion);
            if (typeof raw.updateOffset === 'function') raw.updateOffset();
            slot.setAttachment(raw);
            return true;
        }

        if (SL.RegionAttachment && att instanceof SL.RegionAttachment) {
            const copy = att.copy();
            copy.setRegion(atlasRegion);
            if (typeof copy.updateOffset === 'function') copy.updateOffset();
            slot.setAttachment(copy);
            return true;
        }
        if (SL.MeshAttachment && att instanceof SL.MeshAttachment) {
            if (typeof att.copy === 'function') {
                const meshCopy = att.copy();
                meshCopy.region = atlasRegion;
                if (typeof meshCopy.updateUVs === 'function') meshCopy.updateUVs();
                slot.setAttachment(meshCopy);
                return true;
            }
            att.region = atlasRegion;
            if (typeof att.updateUVs === 'function') att.updateUVs();
            return true;
        }
        return false;
    }

    _applyReplaceSlots(sprite) {
        const list = sprite.replaceSlots;
        if (!list || !list.length) return;
        for (let i = 0; i < list.length; i++) {
            const pair = list[i];
            if (!pair || pair.length < 2) continue;
            const slotName = pair[0];
            const img = BaseAnimation.resolveReplaceSlotImage(pair[1]);
            if (img)
                this.replaceSlotTexture(sprite, slotName, img, 'replace_' + slotName + '_' + i);
        }
    }

    _reapplyRuntimeSlotReplacements(sprite, skeleton) {
        const rep = sprite && sprite._runtimeSlotReplacements;
        if (!rep || !rep.length || !skeleton) return;
        for (let i = 0; i < rep.length; i++) {
            const item = rep[i];
            if (!item || !item.slotName) continue;
            const sl = skeleton.findSlot(item.slotName);
            if (sl && Object.prototype.hasOwnProperty.call(item, 'attachment')) sl.setAttachment(item.attachment);
        }
    }

    _applySkinSwitchPreviewSlotOverrides(sprite, skeleton) {
        if (!sprite || !skeleton) return;
        const hidden = sprite._skinSwitchPreviewHiddenSlots;
        if (hidden && hidden.size > 0) {
            hidden.forEach((slotName) => {
                const sl = skeleton.findSlot(slotName);
                if (sl) sl.setAttachment(null);
            });
        }
        const alphas = sprite._skinSwitchPreviewSlotAlphas;
        if (alphas && typeof alphas.forEach === 'function') {
            alphas.forEach((alpha, slotName) => {
                const sl = skeleton.findSlot(slotName);
                if (sl && sl.color && typeof alpha === 'number') sl.color.a = alpha;
            });
        }
    }
    ...
    update(data) {
        ...
        if (data.unpackPremultipliedAlpha != null) this.unpackPremultipliedAlpha = !!data.unpackPremultipliedAlpha;
    }

    markTextureUnpackForImage(image, nodeOrSkeleton) {
        if (!image) return;
        let unpack = !!this.unpackPremultipliedAlpha;
        if (nodeOrSkeleton != null && nodeOrSkeleton.skeleton != null && nodeOrSkeleton.unpackPremultipliedAlpha != null) {
            unpack = !!nodeOrSkeleton.unpackPremultipliedAlpha;
        }
        if (unpack) {
            skinSwitchSetUnpackUpload(image, true);
        } else {
            skinSwitchSetUnpackUpload(image, false);
        }
    }

    static installGLTextureUnpackPatch() {
        const MARK = '__skinSwitchUnpackByImagePatch';
        function patchProto(proto) {
            if (!proto || typeof proto.update !== 'function' || proto[MARK]) return;
            const orig = proto.update;
            proto.update = function (useMipMaps) {
                const gl = this.context && this.context.gl;
                const img = this._image;
                const unpackPma = gl && gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL;
                let reset = false;
                if (img && skinSwitchDecodedPremultiplyBmps.has(img)) {
                    /* 已在 decode 阶段预乘 */
                } else if (unpackPma != null && img && skinSwitchGetUnpackUpload(img)) {
                    gl.pixelStorei(unpackPma, true);
                    reset = true;
                }
                try {
                    return orig.call(this, useMipMaps);
                } finally {
                    if (reset && unpackPma != null) {
                        gl.pixelStorei(unpackPma, false);
                    }
                }
            };
            proto[MARK] = true;
        ...
        try {
            if (typeof self !== 'undefined') {
                if (self.spine && self.spine.webgl && self.spine.webgl.GLTexture) patchProto(self.spine.webgl.GLTexture.prototype);
                if (self.spine_4 && self.spine_4.GLTexture) patchProto(self.spine_4.GLTexture.prototype);
                if (self.spine_4_1 && self.spine_4_1.GLTexture) patchProto(self.spine_4_1.GLTexture.prototype);
                if (self.spine_4_2 && self.spine_4_2.GLTexture) patchProto(self.spine_4_2.GLTexture.prototype);
                if (self.spine3_8 && self.spine3_8.webgl && self.spine3_8.webgl.GLTexture) patchProto(self.spine3_8.webgl.GLTexture.prototype);
                if (self.spine3_7 && self.spine3_7.webgl && self.spine3_7.webgl.GLTexture) patchProto(self.spine3_7.webgl.GLTexture.prototype);
                if (self.spine_3_5_35 && self.spine_3_5_35.webgl && self.spine_3_5_35.webgl.GLTexture) patchProto(self.spine_3_5_35.webgl.GLTexture.prototype);
            }
        } catch (e) {
            console.warn('BaseAnimation.installGLTextureUnpackPatch', e);
        }
    }

    static wrapAssetManagerUnpackTextureLoader(animInstance) {
        const am = animInstance.spine && animInstance.spine.assetManager;
        if (!am || am.__skinSwitchUnpackWrapped) return;
        if (typeof am.downloadImageBitmap === 'function') {
            const origDl = am.downloadImageBitmap.bind(am);
            am.downloadImageBitmap = function (url, success, error) {
                let request = new XMLHttpRequest();
                request.open('GET', url, true);
                request.responseType = 'blob';
                request.onload = function () {
                    if (request.status === 200 || (request.status === 0 && request.readyState === 4)) {
                        let pma = !!animInstance.unpackPremultipliedAlpha;
                        let optsPma = { premultiplyAlpha: 'premultiply', colorSpaceConversion: 'none' };
                        let finish = function (bm, decodedPremul) {
                            if (decodedPremul) skinSwitchDecodedPremultiplyBmps.add(bm);
                            success(bm);
                        };
                        if (pma && typeof createImageBitmap === 'function') {
                            createImageBitmap(request.response, optsPma).then(function (bm) {
                                finish(bm, true);
                            }).catch(function () {
                                origDl(url, success, error);
                            });
                        } else {
                            origDl(url, success, error);
                        }
                ...
                request.onerror = function () {
                    if (error) error(request.status, request.response);
                };
                request.send();
            };
        }
        const oldLoader = am.textureLoader;
        am.textureLoader = function (image) {
            if (image) {
                let want = !!animInstance.unpackPremultipliedAlpha;
                let isBmp = typeof ImageBitmap !== 'undefined' && image instanceof ImageBitmap;
                if (want && isBmp && skinSwitchDecodedPremultiplyBmps.has(image)) {
                    skinSwitchSetUnpackUpload(image, false);
                } else {
                    skinSwitchSetUnpackUpload(image, want);
                }
            }
            return oldLoader(image);
        };
        am.__skinSwitchUnpackWrapped = true;
    }
...
BaseAnimation.installGLTextureUnpackPatch();
...
class Animation3_6 extends BaseAnimation {
    constructor (pathPrefix, canvas, dpr, isOffscreen) {
        ...
        this.spineLib = spine
        BaseAnimation.wrapAssetManagerUnpackTextureLoader(this)
    };

    createTextureRegion(image, name, nodeForUnpack) {
        if (nodeForUnpack === undefined) nodeForUnpack = null;
        this.markTextureUnpackForImage(image, nodeForUnpack);
            ...
            onload:function(path, data){
                ...
                if (_this.toLoad == 0) {
                    if (_this.errors > 0) {
                        ...
                        thisAnim.spine.assets[_this.filename] = { name: _this.filename, skelType: _this.skelType, textureUnpackPremultipliedAlpha: !!thisAnim.unpackPremultipliedAlpha };
    ...
    prepSpine(filename, autoLoad, toLoadActions) {
        ...
        spineApplyWorldTransform(skeleton, 0);
        ...
        skeleton.state.addListener({
            complete:function(track){
                ...
                if (node) {
                    ...
                    if (node.holdLastFrame && !track.loop) {
                        track.timeScale = 0;
    ...
    playSpine(sprite, position){
        ...
        this._applyReplaceSlots(sprite);
    ...
    render(timestamp) {
        ...
        for (var i = 0; i < nodes.length; i++) {
            ...
            var expandSlotFn = (typeof self !== 'undefined' && self.skinSwitchExpandRendererSlotList) || BaseAnimation.expandRendererSlotList;
            state.hideSlots = expandSlotFn(skeleton, sprite.hideSlots);
            state.update(delta / 1000 * speed);
            state.apply(skeleton);
            this._reapplyRuntimeSlotReplacements(sprite, skeleton);
            this._applySkinSwitchPreviewSlotOverrides(sprite, skeleton);
            spineApplyWorldTransform(skeleton, delta / 1000 * speed);
            ...
            if (renderer.outcropMask) {
                ...
                renderer.clipSlots = expandSlotFn(skeleton, sprite.clipSlots);
            }

            renderer.hideSlots = expandSlotFn(skeleton, sprite.hideSlots);
...
class Animation4_0 extends BaseAnimation{
        ...
        if (skelType === 'json') {
            thisAnim.spine.assetManager.loadText(filename + '.json', () => {
                thisAnim.spine.assetManager.loadTextureAtlas(filename + '.atlas', () => {
                    thisAnim.spine.assets[filename] = { name: filename, skelType: skelType, textureUnpackPremultipliedAlpha: !!thisAnim.unpackPremultipliedAlpha };
                    if (onload) onload()
                }, onerror);
            }, onerror);
        } else {
            thisAnim.spine.assetManager.loadBinary(filename + '.skel', () => {
                ...
                thisAnim.spine.assetManager.loadTextureAtlas(filename + '.atlas', () => {
                    thisAnim.recordSpineAssetTextureUnpack(filename);
                    if (onload) onload();
                }, onerror);
            }, onerror);
        }
    ...
    playSpine(sprite, position){
        ...
        var skeletons = this.spine.skeletons;
        var skeleton;
        if (!(sprite instanceof APNode4_0 && sprite.skeleton && sprite.skeleton.completed)) {
...
class AnimationManager {
    ...
    static spineSkelResourceExistsSync(pathPrefix, filename, ext) {
        ...
        const url = pathPrefix + filename + ext
        try {
            ...
            xhr.send()
            return xhr.status === 200 || xhr.status === 206 || xhr.status === 0
        } catch (e) {
            return false
        }
    }

    resolveSkelFileTypeSync(filename) {
        if (!filename || typeof filename !== 'string') return 'skel'
        if (!this.skelFileTypeCache) this.skelFileTypeCache = {}
        if (this.skelFileTypeCache[filename]) return this.skelFileTypeCache[filename]
        const hasJson = AnimationManager.spineSkelResourceExistsSync(this.pathPrefix, filename, '.json')
        const hasSkel = AnimationManager.spineSkelResourceExistsSync(this.pathPrefix, filename, '.skel')
        let t
        if (hasJson && !hasSkel) t = 'json'
        else if (hasSkel && !hasJson) t = 'skel'
        else if (hasSkel && hasJson) t = 'skel'
        else t = 'skel'
        this.skelFileTypeCache[filename] = t
        return t
    }

    static normalizeSpineExportVersionString(raw) {
        if (!raw || typeof raw !== 'string') return null
        const v = raw
        if (v.indexOf('3.6') === 0) return SupportSpineVersion.v3_6
        if (v.indexOf('3.5') === 0) {
            if (v <= '3.5.35') return SupportSpineVersion.v3_5_35
            return SupportSpineVersion.v3_6
        }
        if (v.indexOf('3.7') === 0) return SupportSpineVersion.v3_7
        if (v.indexOf('3.8') === 0) return SupportSpineVersion.v3_8
        if (v.indexOf('4.0') === 0) return SupportSpineVersion.v4_0
        if (v.indexOf('4.1') === 0) return SupportSpineVersion.v4_1
        if (v.indexOf('4.2') === 0) return SupportSpineVersion.v4_2
        return null
    }

    static detectSpineVersionFromArrayBuffer(arrayBuffer, skelType) {
        if (!arrayBuffer || !arrayBuffer.byteLength) return null
        const sk = (skelType == null ? 'skel' : String(skelType)).toLowerCase()
        if (sk === 'json') {
            let text
            try {
            ...
            if (spineField && spineField[1]) {
                ...
                if (m) return AnimationManager.normalizeSpineExportVersionString(m[0])
                m = inner.match(/\d\.\d+/)
                if (m) return AnimationManager.normalizeSpineExportVersionString(m[0] + '.0')
            }
            const vm = text.match(/\d\.\d+\.\d+/)
            if (vm) return AnimationManager.normalizeSpineExportVersionString(vm[0])
            return null
        }
        ...
        for (let i = 0; i < n; i++) {
            dStr += String.fromCharCode(u8[i])
        }
        const versionReg = /\d\.\d+\.\d+/
        const m = dStr.match(versionReg)
        if (m) return AnimationManager.normalizeSpineExportVersionString(m[0])
        return null
    }

    peekSpineFileVersionSync(filename, skelType) {
        let sk = (skelType == null ? 'skel' : String(skelType)).toLowerCase()
        if (sk !== 'json' && sk !== 'skel') sk = 'skel'
        const ext = sk === 'json' ? '.json' : '.skel'
        const url = this.pathPrefix + filename + ext
        try {
            const xhr = new XMLHttpRequest()
            xhr.open('GET', url, false)
            ...
            return AnimationManager.detectSpineVersionFromArrayBuffer(buf, sk)
        } catch (e) {
            return null
        }
    }

    resolveSkelRuntimeVersion(filename, skelType, explicitVersion, playerVersion) {
        let sk = (skelType == null ? 'skel' : String(skelType)).toLowerCase()
        if (sk !== 'json' && sk !== 'skel') sk = 'skel'
        const key = filename + '|' + sk
        if (explicitVersion != null && explicitVersion !== '') {
            this.skelAutoVersionCache[key] = explicitVersion
            return explicitVersion
        }
        if (this.skelAutoVersionCache[key]) return this.skelAutoVersionCache[key]
        if (playerVersion != null && playerVersion !== '') return playerVersion
        const peeked = this.peekSpineFileVersionSync(filename, sk)
        const v = peeked || SupportSpineVersion.v3_6
        this.skelAutoVersionCache[key] = v
        return v
    }
    ...
    loadAndPlay(sprite, onsuccess, onerror, position) {
        ...
        if (dynamic.hasSpine(sprite.name)) {
            ...
            let stLp = sprite.json === true ? 'json' : (sprite.json === false ? 'skel' : this.resolveSkelFileTypeSync(sprite.name))
            dynamic.loadSpine(sprite.name, stLp, () => {
                let node = dynamic.playSpine(sprite, position)
                if (onsuccess) {
                    onsuccess(node)
                }
