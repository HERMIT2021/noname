...
(function(newDuilib){
	...
	newDuilib.APNode = (function(){
		function APNode(initParam) {
			...
			this.unpackPremultipliedAlpha = !!initParam.unpackPremultipliedAlpha  // 上传纹理 unpack 预乘
	...
	(function skinSwitchInstallSpineGLUnpackByImagePatchDuilib() {
		var MARK = '__skinSwitchUnpackByImagePatch';
		function patchProto(proto) {
			...
			proto.update = function (useMipMaps) {
				var gl = this.context && this.context.gl;
				var reset = false;
				var img = this._image;
				if (gl && gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL != null && img && img.__skinSwitchUnpackPremultiply) {
				...
				try {
					...
					if (reset && gl && gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL != null) {
		...
		try {
			...
			if (root && root.spine && root.spine.webgl && root.spine.webgl.GLTexture) {
				patchProto(root.spine.webgl.GLTexture.prototype);
			}
		} catch (e) {
			console.warn('skinSwitch GLTexture unpack patch (duilib)', e);
		}
	})();
	...
	newDuilib.AnimationPlayer = (function(){
		...
		function AnimationPlayer (pathPrefix, parentNode, elementId) {
			...
			if (gl) {
				...
				var am0 = this.spine.assetManager;
				if (am0 && !am0.__skinSwitchUnpackWrapped) {
					var oldTl = am0.textureLoader;
					var apThis = this;
					am0.textureLoader = function (image) {
						if (image) {
							if (apThis.unpackPremultipliedAlpha) {
								image.__skinSwitchUnpackPremultiply = true;
							} else {
								delete image.__skinSwitchUnpackPremultiply;
							}
						}
						return oldTl(image);
					};
					am0.__skinSwitchUnpackWrapped = true;
				}
			} else {
				this.spine = { assets: {} };
		...
		AnimationPlayer.prototype._markTextureUnpackForImage = function (image, nodeOrSkeleton) {
			if (!image) return;
			var unpack = !!this.unpackPremultipliedAlpha;
		...
		AnimationPlayer.prototype.createTextureRegion = function (image, name, nodeForUnpack) {
			if (nodeForUnpack === undefined) nodeForUnpack = null;
			this._markTextureUnpackForImage(image, nodeForUnpack);
		...
		AnimationPlayer.prototype.prepSpine = function (filename, autoLoad) {
			...
			if (self.spineApplyWorldTransform) self.spineApplyWorldTransform(skeleton, 0); else skeleton.updateWorldTransform();
			...
			skeleton.state.addListener({
				complete:function(track){
					...
					if (node) {
						...
						var finished = !track.loop;
						skeleton.completed = node.holdLastFrame ? false : finished;
						node.completed = node.holdLastFrame ? false : finished;
		...
		AnimationPlayer.prototype.render = function (timestamp) {
			...
			for (var i = 0; i < nodes.length; i++) {
				...
				if (self.spineApplyWorldTransform) self.spineApplyWorldTransform(skeleton, delta / 1000 * speed); else skeleton.updateWorldTransform();
