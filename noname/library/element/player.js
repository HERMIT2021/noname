...
hasSkillTag：`, arg);
  ...
  $draw(num, init, config, cardsetion) {
    ...
    node.style.transitionDuration = get.effectDuration(800, "move", 80) / 1e3 + "s";
    ui.refresh(node);
    ...
    node.listenTransition(function() {
      node.style.transitionDuration = get.effectDuration(500, "move", 80) / 1e3 + "s";
      ui.refresh(node);
      node.delete();
    });
    var that = this;
  ...
  $skill(name2, type, color, avatar) {
    ...
    if (!avatar) {
      ...
      game.broadcastAll(function(arenaFocusTime2) {
        ui.arena.classList.add("playerfocus");
        setTimeout(function() {
          ui.arena.classList.remove("playerfocus");
        }, arenaFocusTime2);
    ...
    setTimeout(
      function() {
        game.broadcastAll(
          function(that2, type2, name3, color2, avatar2, animationTime2) {
            if (lib.config.animation && !lib.config.low_performance) {
              if (game.chess) {
                that2["$" + type2 + "2"](animationTime2);
              } else {
                that2["$" + type2](animationTime2);
              }
            }
            if (name3) {
  ...
  $fullscreenpop(str, nature, avatar, broadcast) {
    ...
    setTimeout(
      ...
      get.effectDuration(avatar ? 1600 : 1e3, "skill", 200)
    );
  }
  /**
   *
