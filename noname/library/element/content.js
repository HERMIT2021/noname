  ...
  async gaincardMultiple(event, trigger, player) {
    ...
    switch (event.animate) {
      case "draw":
        ...
        delay2 = game.delay(0, get.delayx(500, 500), "move");
        break;
      case "gain":
        for (const pair of event.gain_list) {
          if (get.itemtype(pair[1]) == "card") {
        ...
        delay2 = game.delay(0, get.delayx(700, 700), "move");
        break;
      case "gain2":
      case "draw2":
        for (const pair of event.gain_list) {
    ...
    async (event, trigger, player) => {
      ...
      game.broadcastAll(
        function(player2, card, str, id, cardid) {
          ...
          if (game.chess) {
            event2.node = card.copy("thrown", "center", ui.arena);
            event2.node._effectType = "judge";
            event2.node.addTempClass("start");
          } else {
            const node = card.copy();
            node._effectType = "judge";
            event2.node = player2.$throwordered(node, true);
          }
          if (lib.cardOL) {
            lib.cardOL[cardid] = event2.node;
          }
