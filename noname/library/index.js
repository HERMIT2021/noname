...
class Library {
    ...
    effect: {
      ...
      config: {
        ...
        effect_speed_global: {
          ...
          intro: "作为所有可见特效的基础倍速，下面的分类速度会在此基础上继续叠加"
        },
        effect_speed_card: {
          name: "出牌动画速度",
          init: "1",
        ...
        effect_speed_judge: {
          ...
          init: "1",
          item: effectSpeedItems,
          intro: "调整慧识、闪电、乐不思蜀等判定牌翻出、展示、结算停留的速度"
        },
        effect_speed_move: {
          name: "牌移动速度",
          init: "1",
          item: effectSpeedItems,
          intro: "调整摸牌、获得、交给、弃置、牌区移动、元素换位等通用移动动画速度"
        },
        effect_speed_delay: {
          name: "流程停顿速度",
          init: "1",
          item: effectSpeedItems,
          intro: "调整大多数动画后的等待停顿，适合压缩技能和结算之间的空白时间"
        },
        effect_speed_popup: {
          name: "弹出文字速度",
          init: "1",
          item: effectSpeedItems,
          intro: "调整判定生效、伤害数字、胜负、阶段提示等角色上方文字弹出速度"
        },
        effect_speed_player: {
          name: "角色特效速度",
          init: "1",
          item: effectSpeedItems,
          intro: "调整角色入场、离场、聚焦、阵亡等角色相关可见动画速度"
        },
        effect_line_enabled: {
          name: "出牌指示线",
          init: true,
          intro: "关闭后不再显示出牌后指向目标角色的指示线、激光束等目标提示特效"
        },
        effect_speed_line: {
          name: "指示线速度",
          init: "1",
      ...
      config: {
        ...
        show_identity_lord_rating_debug: {
          ...
          onclick() {
            ...
            const top = (data.sorted || []).slice(0, 30).map((name2, index) => `${index + 1}. ${get.translation(name2)} (${name2})：${game.getIdentityLordCharacterRating(name2)}分`).join("\n");
            alert([
              "军争主公评分候选调试",
              "记录时间：" + (data.time || "未知"),
        ...
        edit_identity_lord_character_rating: {
          ...
          onclick() {
            ...
            const addCharacter = function(name2, useCurrentModeFilter = true) {
              if (!name2 || added.has(name2) || !lib.character[name2]) return;
              if (useCurrentModeFilter && lib.filter.characterDisabled(name2)) return;
              added.add(name2);
            ...
            for (const name2 of sourceList) {
              ...
              if (source != name2) {
                ...
                addCharacter(name2, !hasDouzhuanPool);
              }
            }
            if (!hasDouzhuanPool) for (const name2 in lib.characterReplace) {
      ...
      config: {
        ...
        edit_character_rating: {
          ...
          onclick() {
            ...
            const editingRatings = {};
            for (const name2 of characters) {
              editingRatings[name2] = {
                zhu: getDoudizhuRating(storedRatings, name2, "zhu"),
                fan: getDoudizhuRating(storedRatings, name2, "fan")
              };
            }
            const applyPanelStyle = function(node, cssText) {
              node.style.cssText = "position:relative;display:block;box-sizing:border-box;transition:none;text-shadow:none;" + cssText;
            ...
            const createScoreGroup = function(row, name2, role) {
              const group = ui.create.div("", row);
              applyGridStyle(group, "grid-template-columns:repeat(10,24px);gap:4px;align-items:center;");
              const buttons = [];
              for (let score = 1; score <= 10; score++) {
                ...
                applyPanelStyle(button, "height:24px;line-height:24px;text-align:center;border:1px solid rgba(255,255,255,0.16);border-radius:5px;cursor:pointer;font-size:13px;");
                button.listen(function() {
                  editingRatings[name2][role] = score;
              ...
              for (const item of buttons) {
                setButtonActive(item.button, item.score == editingRatings[name2][role]);
              }
            };
            for (const name2 of characters) {
              const row = ui.create.div("", table);
              ...
              createTextNode(nameNode, translatedName, "font-size:16px;line-height:22px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;");
              createTextNode(nameNode, name2, "margin-top:3px;font-size:12px;line-height:16px;color:rgba(247,236,217,0.58);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;");
              createScoreGroup(row, name2, "zhu");
              createScoreGroup(row, name2, "fan");
              rows.push({
                node: row,
            ...
            saveButton.listen(function() {
              ...
              for (const name2 of characters) {
                nextRatings[name2] = {
                  zhu: editingRatings[name2].zhu,
                  fan: editingRatings[name2].fan
                };
              }
  ...
  help = {
    ...
    游戏操作: "<ul><li>长按/鼠标悬停/右键单击显示信息。<li>触屏模式中，下划显示菜单；暂停可在选项-特效中操作。<li>键盘快捷键<br><table><tr><td>A<td>切换托管<tr><td>W<td>切换不询问无懈<tr><td>空格<td>暂停</table><li>编辑牌堆<br>在卡牌包中修改牌堆后，将自动创建一个临时牌堆，在所有模式中共用，当保存当前牌堆后，临时牌堆被清除。每个模式可设置不同的已保存牌堆，设置的牌堆优先级大于临时牌堆。</ul>",
