...
class Game {
  ...
  isCharacterUnlocked(name2) {
    if (!get.config("only_choose_unlocked_character")) return true;
    if (!name2 || !lib.character[name2]) return false;
    return this.getUnlockedCharacters().includes(get.sourceCharacter(name2));
  }
  unlockCharacter(name2) {
    if (!name2 || !lib.character[name2]) return false;
  ...
  filterUnlockedCharacters(list) {
    ...
    return list.filter((name2) => this.isCharacterUnlocked(name2));
  }
  /**
   * @param { string } name
   */
  ...
  zsPlayLineAnimation(name2, node, fake, points) {
    ...
    var finish = function() {
      ...
      if (points != void 0) {
        ...
        var timeS = (animation.fade == true ? Math.max(100, animation.time - fadeBodyTime - 50) : Math.max(100, animation.time - 100)) / 1e3 / 2;
      ...
      animate ??= function(player3) {
        ...
        return drop.finished.then((result) => {
          ...
          const shockWave = wave.animate(
            ...
            {
              duration: get.effectDuration(1e3, "player", 120),
        ...
        for (let i = 0; i < shardCount; i++) {
          ...
          list.push(
            shard.animate(
              ...
              {
                duration: get.effectDuration(1e3 + Math.random() * 500, "player", 120),
