...
import { get, set, load, has } from "../util/config.js";
...
function normalizeCharacterPackList(list) {
...
function saveCharacterPackListIfChanged(name, value, oldValue = get(name)) {
...
function normalizeCharacterPackPreloadConfig() {
  ...
  set("character_pack_preload_default_closed", defaultClosed);
  const savedDisabled = get("character_pack_preload_disabled");
  let disabled = normalizeCharacterPackList(savedDisabled);
  if (!Array.isArray(savedDisabled)) {
    disabled = defaultClosed.slice();
    set("character_pack_preload_disabled", disabled);
...
async function boot() {
  ...
  await security.initSecurity({ lib, game, ui, get: get$1, ai, _status });
  ...
  const disabledCharacterPacks = normalizeCharacterPackList(get("character_pack_preload_disabled"));
  const disabledModePacks = normalizeCharacterPackList(get("mode_preload_disabled"));
  ...
  lib.config.sort_card = get$1.sortCard(lib.config.sort);
...
async function loadConfig() {
  ...
  get("all").plays = [];
  get("all").mode = [];
  if (get("duration") === void 0) set("duration", 500);
  ...
  if (get("effect_speed_popup") === void 0) set("effect_speed_popup", "1");
  if (get("effect_speed_player") === void 0) set("effect_speed_player", "1");
  if (get("effect_line_enabled") === void 0) set("effect_line_enabled", true);
  if (get("effect_speed_line") === void 0) set("effect_speed_line", "1");
  if (get("effect_speed_skill") === void 0) set("effect_speed_skill", "1");
  if (get("swipe_up") === void 0) set("swipe_up", "off");
  if (get("touch_pause_gesture") === void 0) set("touch_pause_gesture", false);
