...
const optionsMenu = function(connectMenu) {
  ...
  var setupCharacterPackPreloadConfig = function() {
    ...
    if (Array.isArray(lib.config.all.sgscharacters)) {
      for (var i2 = 0; i2 < lib.config.all.sgscharacters.length; i2++) addPack(lib.config.all.sgscharacters[i2]);
    }
    if (Array.isArray(lib.config.character_pack_preload_default_closed)) {
      for (var i2 = 0; i2 < lib.config.character_pack_preload_default_closed.length; i2++) addPack(lib.config.character_pack_preload_default_closed[i2]);
    }
    if (Array.isArray(lib.config.character_pack_preload_disabled)) {
      for (var i2 = 0; i2 < lib.config.character_pack_preload_disabled.length; i2++) addPack(lib.config.character_pack_preload_disabled[i2]);
    }
    packList.sort(function(a, b) {
      var aa = lib.translate[a + "_character_config"] || a, bb = lib.translate[b + "_character_config"] || b;
    ...
    for (var i2 = 0; i2 < packList.length; i2++) createPackConfig(packList[i2]);
    ...
    if (Array.isArray(lib.config.all.sgsmodes)) {
      for (var i2 = 0; i2 < lib.config.all.sgsmodes.length; i2++) addMode(lib.config.all.sgsmodes[i2]);
    }
    if (Array.isArray(lib.config.all.stockmode)) {
      for (var i2 = 0; i2 < lib.config.all.stockmode.length; i2++) addMode(lib.config.all.stockmode[i2]);
    }
    for (var modeName in lib.mode) addMode(modeName);
    if (Array.isArray(lib.config.mode_preload_disabled)) {
      for (var i2 = 0; i2 < lib.config.mode_preload_disabled.length; i2++) addMode(lib.config.mode_preload_disabled[i2]);
    }
    modeList.sort(function(a, b) {
      var aa = lib.translate[a] || lib.mode[a]?.name || a, bb = lib.translate[b] || lib.mode[b]?.name || b;
      if (aa != bb) return aa > bb ? 1 : -1;
      return a > b ? 1 : -1;
    });
    var protectedModes = ["identity"];
    var createModePreloadConfig = function(modeName2) {
      var modeLabel = lib.translate[modeName2] || lib.mode[modeName2]?.name || modeName2;
      resourceConfig["mode_preload_" + modeName2] = {
        name: "模式：" + modeLabel,
        init: !lib.config.mode_preload_disabled.includes(modeName2),
        restart: true,
        noSave: true,
        characterPackPreload: true,
        intro: protectedModes.includes(modeName2) ? "身份模式是基础入口，不能关闭" : "关闭后下次启动不会加载该游戏模式，也不会在开始菜单中显示",
        onclick(bool) {
          if (protectedModes.includes(modeName2)) {
            alert("身份模式是基础入口，不能关闭");
            return false;
          }
          if (!Array.isArray(lib.config.mode_preload_disabled)) lib.config.mode_preload_disabled = [];
          if (bool) {
            lib.config.mode_preload_disabled.remove(modeName2);
          } else {
            lib.config.mode_preload_disabled.add(modeName2);
            if (lib.config.mode == modeName2) {
    ...
    for (var i2 = 0; i2 < modeList.length; i2++) createModePreloadConfig(modeList[i2]);
  };
